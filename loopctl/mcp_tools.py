...
"""Generate a default-deny MCP tool list from CLI contract + exposure policy.
    ...
    mcp_tools.py <contract> [--policy <policy>]     print public tool list as JSON
    mcp_tools.py --selftest

The CLI contract remains the single source for argument names, descriptions,
carrier schema, and exit semantics. The policy answers only which declared
commands may an external caller see and under which module/limits. No policy
means no tools. Internal dispatch metadata is returned only by `build()` to the
server and is never printed by this CLI.
"""
...
def schema(command: dict) -> dict:
    ...
    return {
        ...
        "required": [flag.lstrip("-").replace("-", "_") for flag in command["required"]],
...
def build(contract: dict, policy: dict | None = None) -> list[dict]:
    commands = {tool_name(command): command for command in contract["commands"]}
    tools: list[dict] = []
    for entry in validate_policy(policy, contract):
        ...
        tools.append(
            {
                ...
                "_argv": {
                    "loop": command["loop"],
                    "mode": command["mode"],
                    "flags": sorted(set(command["required"]) | set(command["optional"])),
                },
                "_policy": entry,
                **({"_carrier": command["mcp_carrier"]} if command.get("mcp_carrier") else {}),
            }
        )
    return tools
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        return 2
    print(json.dumps({"tools": [public_tool(tool) for tool in tools]}, indent=2, ensure_ascii=False))
    return 0
...
def _selftest() -> int:
    ...
    contract = {
        ...
        "commands": [
            {"loop": "micro", "mode": "run", "target": "t.sh", "required": ["--packet"], "optional": ["--json", "--full"], "opt_in": {"--full": "spends real model turns"}, "io": {"input": {"--packet": "path to a packet"}, "output": ["out/"], "exit": "0 ok · 2 failed"}},
            {"loop": "macro", "mode": "prove", "target": "p.sh", "required": [], "optional": [], "writes": ["receipt.json"]},
            {"loop": "ctg", "mode": "run", "target": "ctg.sh", "required": ["--packet", "--output"], "optional": ["--json"], "mcp_carrier": {"kind": "fixture-inline@1.0.0", "input_schema": {"type": "object", "properties": {"bundle": {"type": "object"}}, "required": ["bundle"], "additionalProperties": False}}},
            {"loop": "ctg", "mode": "build-local", "target": "local.sh", "required": ["--manifest", "--output"], "optional": ["--json"], "mcp_exposed": False},
        ],
    }
    case("no-policy-means-no-tools", build(contract), [])
    policy = {"schema": POLICY_SCHEMA, "tools": [
        {"name": "loopctl_ctg_run", "module": "ctg", "mutation": "disposable-worktree", "network": "none", "secrets": "none", "max_seconds": 60, "max_request_bytes": 1024, "max_output_bytes": 2048},
        {"name": "loopctl_macro_prove", "module": "core", "mutation": "disposable-worktree", "network": "none", "secrets": "none", "max_seconds": 60, "max_request_bytes": 1024, "max_output_bytes": 2048}
    ]}
    tools = build(contract, policy)
    case("only-policy-tools-are-exposed", [tool["name"] for tool in tools], ["loopctl_ctg_run", "loopctl_macro_prove"])
    case("public-tool-hides-internal-fields", any(key.startswith("_") for key in public_tool(tools[0])), False)
    case("carrier-schema-still-comes-from-cli", tools[0]["inputSchema"]["required"], ["bundle"])
    case("module-policy-is-kept-internally", tools[0]["_policy"]["module"], "ctg")
    case("exit-contract-on-every-tool", all("never re-mapped" in tool["description"] for tool in tools), True)
    broken = json.loads(json.dumps(policy)); broken["tools"][0]["name"] = "loopctl_nope_run"
    try: build(contract, broken); case("unknown-policy-tool-is-red", "accepted", "PolicyError")
    except PolicyError: pass
    broken = json.loads(json.dumps(policy)); broken["tools"][0]["name"] = "loopctl_ctg_build-local"
    try: build(contract, broken); case("local-command-cannot-be-reenabled", "accepted", "PolicyError")
    except PolicyError: pass
