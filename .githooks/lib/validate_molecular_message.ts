...
function touchesProtectedSurface(paths: string[]): boolean {
  return paths.some((path) => PROTECTED_PREFIXES.some((prefix) => path.startsWith(prefix)));
}
...
`;
  ...
  const swapped = good
    ...
    .replace("Iteration-Auto-Context: docs/iteration.md", "Iteration-Auto-Context: docs/iteration.md\nFixed-Prompt-Context: docs/prompt.md")
    .replace("MOVED\n", "");
...
function main(argv: string[]): number {
  ...
  if (args.length !== 1 || !messageFile || (flag >= 0 && !changedPathsFile)) {
    console.error(`usage: validate_molecular_message.ts (--selftest | [--changed-paths-file <f>] <commit-msg-file>); got: ${argv.join(" ")}`);
    return 64;
  }
  const paths = changedPathsFile
    ? readFileSync(changedPathsFile, "utf8").split(/\r?\n/).filter(Boolean)
    : stagedPaths();
  return validateFile(messageFile, touchesProtectedSurface(paths));
}
