...
- **`sentinel`** — the packet carries the migrate-injected `unknown` placeholder (any ref with `repo === "unknown"`); sentinel dominates any receipt because `refsStatusForPacket` returns on the shape decision BEFORE any receipt read — no receipt file is ever opened for a placeholder packet, so a corrupt or foreign-schema receipt sitting next to a sentinel packet can never raise `ReceiptCheckError` (src: contracts.ts:78-83).
...
- A receipt that exists but cannot be parsed as JSON, OR parses but declares an unsupported `schema_version`, is not a status at all: both throw the typed `ReceiptCheckError`, which `cli.ts` maps to exit 2 with a named diagnostic — "the evidence exists but cannot be read under this contract … a check that ran and failed, not a generic input error" (src: contracts.ts:63-65, 86-90; loop_wiki/evolve-perfect-seed-repo-factory/src/cli.ts:123). The schema-mismatch branch was added alongside `trigger.sh`'s explicit `stale` case (below) so that "audited-then-broken" reads distinctly from a bare parse failure at every layer, not just at the shell.

## resolve-refs — the explicit peer audit

`cli.ts resolve-refs --packet <p> --peer <absolute-path>` verifies each ref's commit exists (`git cat-file -e <commit>^{commit}`) and each path is tracked at that commit (`git ls-tree -r --name-only`) in the peer clone, then writes the receipt with packet hash, peer, ref count (src: loop_wiki/evolve-perfect-seed-repo-factory/src/cli.ts:37-81). Without `--peer` it prints `NOT_RUN: resolve-refs requires --peer <absolute-path>; refs were not audited` and exits 2 — never PASS (src: cli.ts:44-45). `cli.ts refs-status --packet <p>` emits the status JSON (`perfect-seed-refs-status@1.0.0`) that `trigger.sh` relays verbatim into exchange context and route result (src: cli.ts:26-35; trigger.sh:19-21, 30-43, 70-85). The delivery guard itself is a four-branch `case "$REFS_STATUS"`: exactly `declared`, `sentinel`, and `resolved` proceed to delivery, while the `stale` branch and the unrecognized catch-all both **exit 2** (src: loop_wiki/evolve-perfect-seed-repo-factory/trigger.sh:22-29). `trigger.sh` FAILs the `stale` status by name — a distinct message naming the resolve-receipt mismatch, never lumped into the generic "unrecognized refs_status" branch — so a delivery blocked by a broken audit reads differently from one blocked by a typo'd status (src: trigger.sh:24-28). The operational remedy is stated in the FAIL message itself: rerun `resolve-refs --peer` against the peer clone to re-bind the receipt to the current packet bytes, or remove the stale `<packet>.resolve-receipt.json` to deliver as `declared` — never edit the receipt to make it match (src: trigger.sh:25-27). Both the `stale` case and the catch-all fail CLOSED rather than relaying the status onward, and the reason is downstream carriage: `trigger.sh` copies `refs_status` verbatim into the route result AND into the wiki-update request's `route_result` block (src: trigger.sh:80, 138), so an unrecognized status relayed instead of refused would be laundered into the human gate's evidence and the digestion station's payload as if it were a known audit state (src: trigger.sh:22-29).
...
The refs lifecycle is pinned behavior-by-behavior in `tests/seed_factory.test.ts`: "rejects a packet without source_refs" (:299) and "rejects malformed source_refs shapes" (:307) at validate; "resolve-refs without --peer reports NOT_RUN with exit 2" (:325) and "resolve-refs audits commit existence and tracked path against an explicit peer" (:334) for the audit; "refs-status reports declared for shape-valid refs and sentinel for migrated placeholders" (:384); "resolved status is granted only by a receipt bound to the audited packet bytes" (:405); "a receipt missing its binding fields reads stale, never declared or resolved" (:443); "a corrupted receipt fails the check with exit 2 and a named diagnostic" (:456); "migrated legacy packet builds with sentinel refs and refs_status sentinel" (:468); "a receipt under a foreign schema fails the check with exit 2, same taxonomy as corruption" (:478, ISSUE-23); and "trigger rejects a stale refs_status by name, not as unrecognized" (:491, ISSUE-23) (src: loop_wiki/evolve-perfect-seed-repo-factory/tests/seed_factory.test.ts).
