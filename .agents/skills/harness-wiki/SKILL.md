...
| **N-variant 執行反哺迴圈**(sparse/blind 任務;oracle-gate=harness-spec §4.5) | [loop-harness-standard execution-feedback 模組](../loop-harness-standard/modules/execution-feedback.md) + `loop_wiki/engine_nv.sh`(兩相位 wrapper,組合 engine.sh 不改本體) | 計劃斷言表(B-1 pre-registered)→ N worktree 各跑未改動 engine.sh → `<loop>/_nv-run/round<K>/`(APPROACH+軌跡+judge-materials)→ 編排 session 派 fresh Opus 判官出 verdict → winner.diff(exit 0)/plan-delta(exit 10)SURFACE | dispatch 前假多樣機械擋 + verdict 機械勾稽(HELD 必附軌跡引用/plan-dir 乾淨/rerun 必過 no-smuggled)+ 逐斷言 HELD/REFUTED/UNOBSERVED + 人 admit(勝者落主 tree 與 plan-delta 皆人閘) | **selftest 已證**(2026-07-19 落地:good/hollow 全過,含 N=2 真 engine conform_only 煙測+收割三步;端到端真案例未跑) |
...
| N-variant 執行反哺 | `variant-*/APPROACH.md`+判官材料=`judge-materials.md`(**由 `loop_wiki/engine_nv.sh` 內嵌模板生成,script 即 SSOT**);斷言表=該計劃 `NN-slice.md`(B-1 schema,定義權=harness-spec §4.5) | exit 碼契約=`loop_wiki/engine_nv.sh` 頭注(枚舉不複列)+4 checker=`loop-harness-standard/scripts/execution-feedback/` |
