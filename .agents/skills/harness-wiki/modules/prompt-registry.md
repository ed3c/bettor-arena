...
| N-variant execution feedback | assertion table in the plan slice using `harness-spec.md §4.5` B-1 schema | **no materializer** — the wrapper that was going to write `variant-*/APPROACH.md` and `judge-materials.md` never existed and was retired 2026-08-07 (#26); a real run defines them in its own Workflow script | `loop-harness-standard/scripts/execution-feedback/` checkers (the wrapper's exit contract is gone with it) | `loop-harness-standard/modules/execution-feedback.md` |
...
| technical-equivalence research | `loop_wiki/evolve-technical-equivalence-research/PROMPT.md` owns the task contract; `profile/technical-equivalence.md` owns the fixed research profile | `equivalence.py` compiles digest-bound primary/gap Gemini prompts and writes immutable `_runs/` packets; raw provider material stays runtime-only | deterministic grounding in `equivalence.py` → fresh-zero-context judge packet/result → external Human admit; `prove_equivalence.sh` records traversal and `control_equivalence_entry.sh` independently tests committed behavior | `ROUTES.md`, `modules/architecture.md`, `modules/exchange-formats.md`, `modules/production-readiness.md`; offline/live/judge/Human states never proxy one another |
...
| N-variant judge materials | **retired 2026-08-07 (#26)** — the named owner `loop_wiki/engine_nv.sh` never existed | checker fixtures and verdict file only |
