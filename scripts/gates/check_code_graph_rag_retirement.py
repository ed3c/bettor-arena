...
def snapshot(root=Path(".")):
    ...
    manifest = (
        root / "docs/knowledge-providers/providers/code-graph-rag.json"
    ).exists()
    provider_registry = load(root / "docs/knowledge-providers/registry.json")
    participant = (
        root / "docs/knowledge-providers/evals/participants/code-graph-rag.json"
    ).exists()
    evaluator = (root / "scripts/knowledge_provider_eval_registry.py").read_text(
        encoding="utf-8"
    )
    history = (root / "docs/knowledge-providers/alternatives.md").read_text(
        encoding="utf-8"
    )
...
def validate_values(
    seq, manifest, provider_registry, participant, evaluator, history, doc, readme
):
    ...
    req(
        all(
            "stage-13-code-graph-rag" not in x["prerequisite_items"]
            for x in seq["items"]
        ),
        "legacy prerequisite active",
    )
    req(not manifest, "retired provider manifest remains in active provider directory")
    entries = provider_registry.get("providers", [])
    req(
        all(x.get("id") != "code-graph-rag" for x in entries),
        "retired provider remains in provider registry",
    )
    ...
    req(
        "HISTORICAL_PROVIDERS" not in evaluator,
        "retired provider retains evaluator registry class",
    )
    req(
        "Code-Graph-RAG" in history and "RETIRED" in history, "decision history missing"
    )
...
def validate(root=Path(".")):
    return validate_values(*snapshot(root))


def selftest(root=Path(".")):
    vals = list(snapshot(root))
    validate_values(*vals)
    names = []
    tests = []
    a = copy.deepcopy(vals)
    next(x for x in a[0]["items"] if x["order"] == 13)["issues"] = [41]
    tests.append(("queue-41", a))
    a = copy.deepcopy(vals)
    a[1] = True
    tests.append(("manifest-present", a))
    a = copy.deepcopy(vals)
    a[2]["providers"].append(
        {
            "id": "code-graph-rag",
            "path": "providers/code-graph-rag.json",
            "digest": "sha256:" + "0" * 64,
        }
    )
    tests.append(("provider-registry", a))
    a = copy.deepcopy(vals)
    a[3] = True
    tests.append(("participant-active", a))
    a = copy.deepcopy(vals)
    a[4] = a[4].replace(
        'PROVIDERS = {"serena", "grepai", "mem0"}',
        'PROVIDERS = {"serena", "grepai", "code-graph-rag", "mem0"}',
    )
    tests.append(("provider-set", a))
    a = copy.deepcopy(vals)
    a[4] += '\nHISTORICAL_PROVIDERS = {"code-graph-rag"}\n'
    tests.append(("historical-evaluator-class", a))
    a = copy.deepcopy(vals)
    a[5] = a[5].replace("**RETIRED**", "historical")
    tests.append(("history-marker", a))
    for name, a in tests:
        try:
            validate_values(*a)
        except E:
...
def main():
    ...
    ns = ap.parse_args()
    try:
        out = selftest() if ns.selftest else validate()
    except (E, OSError, json.JSONDecodeError) as exc:
