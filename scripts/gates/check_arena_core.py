...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="check_arena_core.py")
    parser.add_argument("--root", type=Path)
    parser.add_argument("--selftest", action="store_true")
    args = parser.parse_args(argv)
    root = args.root.resolve() if args.root else Path(__file__).resolve().parents[2]
    if not (root / "AGENTS.md").is_file():
        print("arena-core FATAL: repository root not found", file=sys.stderr)
        return EXIT_FATAL
    suffix = ["--selftest"] if args.selftest else []
    for command in [
        [sys.executable, "scripts/gates/check_agent_docs.py", *suffix],
        [
            sys.executable,
        ...
        if status != EXIT_OK:
            return status
    print("PASS arena-core aggregate" + (" selftest" if args.selftest else ""))
    return EXIT_OK
