...
def validate(packet: dict[str, Any], binding: dict[str, Any]) -> list[str]:
    ...
    if admission.get("consumer_live_canary") != "NOT_EXERCISED": errors.append("consumer live canary falsely promoted")
    if admission.get("git_town_darwin_artifact") != "HUMAN_ADMIT_REQUIRED": errors.append("Git Town Darwin Human Admit erased")
    if admission.get("forgejo_service_activation") != "NOT_EXERCISED": errors.append("Forgejo activation falsely promoted")
    if admission.get("merge_or_ship_authority") is not False: errors.append("merge/ship authority must remain false")
    lanes = packet.get("evidence_lanes", {})
    for lane in ("bettor_worker_processes", "bettor_linked_worktrees", "bettor_checkpoint_resume", "bettor_stale_result_refusal", "bettor_budget_reconciliation", "grepai", "scip_lsp", "tree_sitter", "serena", "sqlite", "lancedb", "git_town", "forgejo", "github_exact_head"):
        if lanes.get(lane) != "NOT_EXERCISED": errors.append(f"{lane} must remain NOT_EXERCISED before local canary")
    for lane in ("runtime_env_synthetic_scheduler", "runtime_env_synthetic_worktrees"):
        if lanes.get(lane) != "PASS": errors.append(f"{lane} must preserve observed synthetic PASS")
    forbidden = packet.get("forbidden", {})
    for key in ("static_fixture_as_consumer_pass", "synthetic_runtime_as_bettor_pass", "arbitrary_model_shell_command", "auto_admit_darwin_git_town_artifact", "auto_activate_forgejo", "auto_merge_ship_force_push"):
        if forbidden.get(key) is not True: errors.append(f"forbidden control disabled: {key}")
    return errors
...
def selftest(packet: dict[str, Any], binding: dict[str, Any]) -> list[str]:
    ...
    for name, mutate, needle in cases:
        p, b = copy.deepcopy(packet), copy.deepcopy(binding)
        mutate(p, b)
...
def main() -> int:
    ...
    try: packet, binding = load(PACKET), load(BINDING)
    ...
    if errors:
        ...
        return 1
    print(f"PASS: issue #161 runtime admission is fail-closed ({packet['admission']['scheduler_runtime']})")
    return 0
