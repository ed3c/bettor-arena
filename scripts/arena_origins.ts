...
function portableRelative(path: string, label: string): string {
  ...
  if (!normalized || parts.some((part) => !part || part === "..")) throw new EnvironmentContractError(`${label}: path escape`);
  return normalized;
}

export function validateContract(value: unknown, options: { allowLocalRepositories?: boolean } = {}): OriginContract {
  closedObject(value, ["schema", "logical_source", "release_manifest", "accepted_equivalence", "origins"], "origin contract");
  if (value.schema !== CONTRACT_SCHEMA) throw new EnvironmentContractError(`origin contract: schema must be ${CONTRACT_SCHEMA}`);
  if (typeof value.logical_source !== "string" || !value.logical_source.trim()) throw new EnvironmentContractError("origin contract: logical_source required");
  const releaseManifest = portableRelative(String(value.release_manifest), "origin contract.release_manifest");
  const accepted = uniqueStrings(value.accepted_equivalence, "origin contract.accepted_equivalence") as EquivalenceMode[];
  if (!accepted.length || accepted.some((mode) => !MODES.includes(mode))) throw new EnvironmentContractError("origin contract: unsupported equivalence mode");
  if (!Array.isArray(value.origins) || value.origins.length !== 2) throw new EnvironmentContractError("origin contract: exactly two origins required for MVP");
  const ids = new Set<string>();
  const roles = new Set<string>();
  const origins = value.origins.map((item, index) => {
    closedObject(item, ["id", "role", "scopes", "repository", "live_probe"], `origin[${index}]`);
    if (typeof item.id !== "string" || !ID.test(item.id) || ids.has(item.id)) throw new EnvironmentContractError(`origin[${index}]: invalid or duplicate id`);
    ids.add(item.id);
    if (item.role !== "authoring" && item.role !== "distribution") throw new EnvironmentContractError(`origin ${item.id}: invalid role`);
    ...
    if (typeof item.repository !== "string" || !item.repository.trim()) throw new EnvironmentContractError(`origin ${item.id}: repository required`);
    const repository = item.repository;
    const isLocalPath = isAbsolute(repository) || repository.startsWith("file://");
    if (isLocalPath && !options.allowLocalRepositories) throw new EnvironmentContractError(`origin ${item.id}: local path is not a versioned repository identity`);
    if (!isLocalPath) {
      let url: URL;
      try { url = new URL(repository) } catch { throw new EnvironmentContractError(`origin ${item.id}: repository must be a URL`) }
      if (url.username || url.password) throw new EnvironmentContractError(`origin ${item.id}: credentials are forbidden in repository URL`);
      if (url.hostname === "localhost" && item.role !== "authoring") throw new EnvironmentContractError("localhost may only be the local authoring origin");
    }
    if (item.role === "distribution") {
      if (!scopes.includes("cloud") || item.live_probe !== "ci") throw new EnvironmentContractError("distribution origin must be cloud-scoped and CI-probed");
  ...
  if (!roles.has("authoring") || !roles.has("distribution")) throw new EnvironmentContractError("origin contract requires authoring and distribution roles");
  return { schema: CONTRACT_SCHEMA, logical_source: value.logical_source, release_manifest: releaseManifest, accepted_equivalence: accepted, origins };
}
...
function manifestDigest(text: string, label: string): string {
  try { return sha256Json(JSON.parse(text)) }
  catch { throw new EnvironmentContractError(`${label}: release manifest is not JSON`) }
}

function fetchSubject(repository: string, commit: string, manifestPath: string): OriginReceipt["subject"] {
  if (!SHA40.test(commit)) throw new EnvironmentContractError("origin probe requires an exact 40-hex commit, never main/HEAD");
  ...
  try {
    ...
    return { commit, tree, release_manifest: manifestPath, release_manifest_sha256: manifestDigest(manifest, repository) };
...
function validateReceipt(receipt: OriginReceipt, label: string): void {
  ...
  if (receipt.status === "PASS") {
    if (!receipt.subject || !SHA40.test(receipt.subject.commit) || !SHA40.test(receipt.subject.tree)) throw new EnvironmentContractError(`${label}: PASS lacks exact Git subject`);
  } else if (receipt.subject !== null) throw new EnvironmentContractError(`${label}: non-PASS receipt carries a fake subject`);
}
...
export function compareOrigins(contract: OriginContract, receipts: OriginReceipt[]): EquivalenceReceipt {
  ...
  if (new Set(receipts.map((receipt) => receipt.origin.id)).size !== 2 || new Set(receipts.map((receipt) => receipt.origin.role)).size !== 2) {
    throw new EnvironmentContractError("equivalence receipts must represent distinct origins and roles");
  }
  if (receipts.some((receipt) => receipt.logical_source !== contract.logical_source)) throw new EnvironmentContractError("equivalence logical source mismatch");
  const [left, right] = receipts.map((receipt) => receipt.subject!);
  let mode: EquivalenceMode | null = null;
  let subject: Record<string, string> | null = null;
  if (left.commit === right.commit) { mode = "exact-commit"; subject = { commit: left.commit } }
  else if (left.tree === right.tree) { mode = "same-tree"; subject = { tree: left.tree } }
...
export function status(root: string): Record<string, unknown> & { content_sha256: string } {
  ...
  if (!existsSync(manifestPath)) throw new EnvironmentContractError(`release manifest absent: ${contract.release_manifest}`);
  ...
  return withDigest({
    ...
    origins: contract.origins.map((origin) => ({
      ...
      note: origin.live_probe === "ci" ? "Requires an exact-head distribution probe." : "Requires a trusted local Forgejo host probe.",
    })),
...
export function selftest(): number {
  ...
  try {
    ...
    const fixture = validateContract({
      ...
      origins: [
        { id: "fixture-authoring", role: "authoring", scopes: ["local", "promotion"], repository: originA, live_probe: "host-only" },
        { id: "fixture-distribution", role: "distribution", scopes: ["cloud", "promotion"], repository: originB, live_probe: "ci" },
      ],
    ...
    if (compareOrigins(fixture, [a1, b1]).mode !== "exact-commit") throw new EnvironmentContractError("selftest exact-commit failed");
    ...
    if (compareOrigins(fixture, [a1, b2]).mode !== "same-tree") throw new EnvironmentContractError("selftest same-tree failed");
    ...
    if (compareOrigins(fixture, [a1, b3]).mode !== "same-release-manifest") throw new EnvironmentContractError("selftest same-release-manifest failed");
    ...
    if (probeOrigin(fixture, "fixture-authoring", "main").status !== "ABSENT") throw new EnvironmentContractError("selftest mutable ref was accepted");
...
function parse(argv: string[]): Options {
  ...
  while (rest.length) {
    ...
    if (["check", "status", "probe", "equivalence"].includes(token)) { options.command = token as Options["command"]; continue }
    if (token === "--selftest") { options.selftest = true; continue }
    const key = token.startsWith("--") ? token.slice(2) : "";
    if (!["root", "output", "origin", "commit", "left", "right"].includes(key)) throw new EnvironmentContractError(`unknown argument: ${token}`);
...
export async function main(argv = process.argv.slice(2)): Promise<number> {
  try {
    ...
    if (options.command === "check") {
      ...
      if (!options.output || !options.origin || !options.commit) throw new EnvironmentContractError("probe requires --origin, --commit, and --output");
      const receipt = probeOrigin(contract, options.origin, options.commit);
      writeJson(resolve(options.output), receipt);
      if (receipt.status !== "PASS") { console.error(`ORIGIN-RED ${receipt.note}`); return 2 }
    } else if (options.command === "equivalence") {
      if (!options.output || !options.left || !options.right) throw new EnvironmentContractError("equivalence requires --left, --right, and --output");
      const receipt = compareOrigins(contract, [readJson<OriginReceipt>(resolve(options.left)), readJson<OriginReceipt>(resolve(options.right))]);
      writeJson(resolve(options.output), receipt);
