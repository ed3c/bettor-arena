...
def validate_activation(root: Path, receipt: dict[str, Any]) -> None:
    ...
    require(
        value.get("schema") == "bettor-arena/provider-activation-receipt/v1",
        "activation schema drift",
    )
    require(value.get("outcome") == "ADMITTED", "provider activation is not ADMITTED")
    authority = value.get("authority", {})
    require(
        authority.get("activated_provider") is True,
        "provider activation authority absent",
    )
    require(
        value.get("cleanup", {}).get("status") == "PASS", "activation cleanup not PASS"
    )
    ...
    require(
        ...
        and {p.get("provider_id") for p in providers} == {"serena", "grepai"},
        "activation provider set drift",
    )
    for provider in providers:
        require(
            provider.get("state_after") == "ADMITTED",
            f"{provider.get('provider_id')}: not ADMITTED",
        )
        canary_path = provider.get("canary_receipt_path")
        require(
            isinstance(canary_path, str) and canary_path, "canary receipt path missing"
        )
        canary = load_json(root / canary_path)
        require(
            canary.get("status") == "PASS",
            f"{provider.get('provider_id')}: canary not PASS",
        )
        require(
            canary.get("execution", {}).get("executed") is True,
            f"{provider.get('provider_id')}: canary not executed",
        )
        require(
            canary.get("execution", {}).get("state") == "PASS",
            f"{provider.get('provider_id')}: execution not PASS",
        )
        require(
            canary.get("cleanup", {}).get("status") == "PASS",
            f"{provider.get('provider_id')}: cleanup not PASS",
        )
        require(
            canary.get("manifest", {}).get("identity_match") is True,
            f"{provider.get('provider_id')}: manifest identity mismatch",
        )
        require(
            canary.get("manifest", {}).get("identity_state") == "PINNED",
            f"{provider.get('provider_id')}: manifest not PINNED",
        )
        require(
            canary.get("source_readback", {}).get("token_observed") is True,
            f"{provider.get('provider_id')}: source readback absent",
        )
...
def validate_transition(
    ...
    *,
    ...
    require(
        receipt.get("schema") == "bettor-arena/pdf-terminal-advancement/v1",
        "advancement receipt schema drift",
    )
    ...
    if check_blob:
        require(
            queue_blob(root / QUEUE) == receipt.get("queue_blob_before_git_sha"),
            "stale queue subject",
        )
    ...
    require(
        ...
        "predecessor/successor missing",
    )
    current = active[0]
    require(current.get("id") == predecessor.get("id"), "wrong active predecessor id")
    require(
        current.get("order") == predecessor.get("order"),
        "wrong active predecessor order",
    )
    require(
        predecessor.get("issue") in current.get("issues", []),
        "wrong active predecessor issue",
    )
    order = current["order"]
    require(order < 24, "controller cannot advance final convergence")
    require(
        successor.get("order") == order + 1, "successor must be immediate next order"
    )
    next_item = items[order + 1]
    require(next_item.get("id") == successor.get("id"), "wrong successor id")
    require(
        successor.get("issue") in next_item.get("issues", []), "wrong successor issue"
    )
    require(
        next_item.get("queue_state") == "BLOCKED_BY_PREDECESSOR",
        "successor is not blocked",
    )
    require(
        current.get("id") in next_item.get("prerequisite_items", []),
        "successor does not depend on predecessor",
    )
    for item in items[:order]:
        require(
            item.get("queue_state") == "COMPLETE",
            f"earlier stage {item.get('order')} is not COMPLETE",
        )
    for item in items[order + 1 : 25]:
        require(
            item.get("queue_state") == "BLOCKED_BY_PREDECESSOR",
            f"later stage {item.get('order')} already advanced",
        )
    summary = queue.get("current", {})
    require(summary.get("active_order") == order, "current.active_order drift")
    require(
        summary.get("active_issue") == predecessor.get("issue"),
        "current.active_issue drift",
    )
    ...
    require(
        remote.get("head_sha") == "76eca27c8a4eda7623d498d8a9bdc3365b26e901",
        "completion PR head drift",
    )
    require(
        remote.get("merge_commit_sha") == "3357dadcde509a7e92515be52d9b301e4ff130e0",
        "completion merge drift",
    )
...
def update_docs(root: Path) -> None:
    ...
    replacements = [
        ...
        (
            "ordered live-acceptance queue is stopped at #92",
            "ordered live-acceptance queue is now active at #140",
        ),
        (
            "governance `IMPLEMENTED`; active #92",
            "governance `IMPLEMENTED`; active #140",
        ),
        (
            "PR #124 merged; live provider #92 active",
            "PR #124 merged; live provider #92 complete; #140 active",
        ),
    ]
...
def validate_transition_after(
    queue: dict[str, Any], receipt: dict[str, Any]
) -> dict[str, Any]:
    predecessor = receipt["predecessor"]
    successor = receipt["successor"]
    ...
    require(
        queue["current"]["active_order"] == successor["order"],
        "active order not advanced",
    )
    require(
        queue["current"]["active_issue"] == successor["issue"],
        "active issue not advanced",
    )
    require(
        sum(1 for item in queue["items"] if item["queue_state"] == "ACTIVE") == 1,
        "post-apply ACTIVE cardinality drift",
    )
    return {
        "status": "PASS",
        "active_order": successor["order"],
        "active_issue": successor["issue"],
    }
...
def selftest(root: Path, receipt_path: Path) -> dict[str, Any]:
    ...
    r["predecessor"]["issue"] = 999999
    must_fail("wrong-predecessor", copy.deepcopy(queue), r)
    r = copy.deepcopy(receipt)
    r["successor"]["order"] += 1
    must_fail("skipped-order", copy.deepcopy(queue), r)
    r = copy.deepcopy(receipt)
    r["outcome"] = "REJECTED"
    must_fail("unadmitted-receipt", copy.deepcopy(queue), r)
    q = copy.deepcopy(queue)
    q["items"][13]["queue_state"] = "ACTIVE"
    must_fail("two-active-items", q, copy.deepcopy(receipt))
    q = copy.deepcopy(queue)
    q["items"][12]["order"] = 24
    must_fail("final-advance", q, copy.deepcopy(receipt))
    require(
        receipt.get("queue_blob_before_git_sha") == queue_blob(root / QUEUE),
        "stale queue control baseline",
    )
    ...
    try:
        if args.command == "plan":
            out = validate_transition(
                load_json(QUEUE), load_json(receipt_path), root=root
            )
