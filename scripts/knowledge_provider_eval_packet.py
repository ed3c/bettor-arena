...
def authority(value: Any, label: str) -> None:
    require(
        isinstance(value, dict) and value.get("candidate_only") is True,
...
def validate_packet(
    value: Any, suite: dict[str, dict], people: dict[str, dict]
) -> dict:
    oid = value.get("observation_id")
    cid = value.get("case_id")
    pid = value.get("participant_id")
    require(
        value.get("schema_version") == "knowledge-provider-eval-observation/v1",
        "observation schema",
    )
    allowed = {
        "schema_version",
    ...
    require(
        value.get("participant_identity_digest") == person["identity_digest"],
        f"{oid}: participant identity drift",
    )
    if person["kind"] == "provider":
        require(
            value.get("provider_manifest_digest") == person["manifest_digest"],
            f"{oid}: provider manifest drift",
        )
    else:
        require(
            value.get("provider_manifest_digest") is None, f"{oid}: control manifest"
        )
    require(value.get("subject") == case["subject"], f"{oid}: subject drift")
    validate_subject(value.get("subject"), f"{oid}.subject")
    require(
        value.get("query_digest") == case["query_digest"],
        f"{oid}: query digest mismatch",
    )
    ...
    require(
        state in {"PASS", "FAIL", "ABSENT", "NOT_EXERCISED", "SKIPPED_BY_POLICY"},
        f"{oid}: state",
    )
    ...
    require(
        index.get("state") in {"FRESH", "STALE", "UNKNOWN", "NOT_APPLICABLE"},
        f"{oid}: index",
    )
    ...
    for item in results:
        ...
        require(
            rid not in ids and isinstance(rank, int) and rank > 0 and rank not in ranks,
            f"{oid}: duplicate result",
        )
        ids.add(rid)
        ranks.add(rank)
        ...
        require(
            verification
            in {"SOURCE_READBACK_CONFIRMED", "CANDIDATE_ONLY", "UNRESOLVED"},
            f"{oid}: verification",
        )
        if verdict == "FOUND" and case["hard_gates"]["source_readback_required"]:
            require(
                verification == "SOURCE_READBACK_CONFIRMED" and item.get("source_refs"),
                f"{oid}: FOUND without source readback",
            )
        if verdict == "NO_FLOW":
            require(
                rid not in case["oracle"]["must_remain_unknown"],
                f"{oid}: coverage gap collapsed to NO_FLOW",
            )
