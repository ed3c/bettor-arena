...
def _require_repo_relative(value: str, field: str) -> None:
    ...
    if (
        os.path.isabs(value)
        or value.startswith("~")
        or any(h in value for h in HOME_PATTERNS)
    ):
...
def load_manifest(path: Path) -> dict:
    ...
    for component in data["components"]:
        ...
        _require_repo_relative(
            component["source"], f"components[{component['id']}].source"
        )
        _require_repo_relative(
            component["target"], f"components[{component['id']}].target"
        )
        for prefix in component.get("exclude_prefixes", []):
            _require_repo_relative(
                prefix, f"components[{component['id']}].exclude_prefixes"
            )
...
def build_operations(
    source_root: Path, target_root: Path, manifest: dict
) -> list[Operation]:
    ...
    for component in manifest["components"]:
        ...
        rels = [
            ...
            for r in rels
            if not any(
                _prefix_match(r, p) for p in component.get("exclude_prefixes", [])
            )
            and not any(part in excluded_parts for part in Path(r).parts)
        ]
        if not rels and component.get("required", False):
            raise ValueError(
                f"required migration component empty or missing: {component['source']}"
            )
        for rel in sorted(rels):
            ...
            if component["kind"] == "file":
                ...
                target = (
                    target_root
                    / component["target"]
                    / Path(rel).relative_to(source_prefix)
                )
            ...
            if source.is_symlink():
                ...
                if os.path.isabs(link):
                    raise ValueError(
                        f"absolute symlink in payload re-couples the target: {rel} -> {link}"
                    )
                operations.append(
                    Operation(
...
def apply_operations(
    operations: list[Operation], manifest: dict, source_root: Path
) -> int:
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        if args.apply:
            ...
            payload["allowlist_entries_added"] = ensure_target_allowlist(
                target_root, manifest
            )
            ...
            if gate_status == "fail":
                print(
                    f"FAIL: target gate {TARGET_GATE_REL} red after apply",
                    file=sys.stderr,
                )
                return 2
        if args.stats:
            ...
            print(
                f"{mode.upper()}: {payload['files_planned']} files planned "
                f"({payload['rewrite_planned']} rewrite, {payload['evidence_planned']} evidence)"
                + (
                    f", {payload['files_written']} written"
                    if args.apply
                    else ", 0 bytes written"
                )
            )
        return 0
...
def _selftest() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        check(
            "dry-run-exit", dry.returncode == 0, f"exit {dry.returncode}: {dry.stderr}"
        )
        check(
            "dry-run-no-writes",
            _snapshot(tgt) == before,
            "target tree changed on dry-run",
        )
        planned = -1
        if dry.returncode == 0:
            ...
            check(
                "dry-run-symlink-planned",
                payload["symlink_planned"] == 1,
                f"symlink_planned={payload.get('symlink_planned')}",
            )
            check(
                "dry-run-rewrite-planned",
                payload["rewrite_planned"] == 3,
                f"rewrite_planned={payload['rewrite_planned']}",
            )
            check(
                "dry-run-evidence-planned",
                payload["evidence_planned"] == 1,
                f"evidence_planned={payload['evidence_planned']}",
            )
            check(
                "dry-run-no-abs-paths",
                src_str not in dry.stdout,
                "payload leaks source root",
            )
        ...
        check(
            ...
            f"exit {applied.returncode}: {applied.stderr}",
        )
        if applied.returncode == 0:
            payload = json.loads(applied.stdout)
            check(
                "apply-count-matches-dry-run",
                payload["files_written"] == planned,
                f"written={payload['files_written']} planned={planned}",
            )
            tool = (tgt / "code/tool.py").read_text(encoding="utf-8")
            check(
                "rewrite-token",
                ROOT_TOKEN_DEFAULT in tool and src_str not in tool,
                f"tool.py content: {tool!r}",
            )
            check(
                "evidence-verbatim",
                (tgt / "evidence/trace.md").read_bytes()
                == (src / "evidence/trace.md").read_bytes(),
                "evidence file was rewritten",
            )
            check(
                "excluded-part-skipped",
                not (tgt / "code/node_modules/skip.py").exists(),
                "excluded_path_parts file migrated",
            )
            check(
                "exec-bit",
                (tgt / "bin/run.sh").stat().st_mode & stat.S_IXUSR != 0,
                "exec bit lost",
            )
            alias = tgt / "code/alias_docs"
            check(
                "symlink-recreated",
                alias.is_symlink() and os.readlink(alias) == "../docs",
            ...
            if receipt.is_file():
                check(
                    "receipt-no-abs-paths",
                    src_str not in receipt.read_text(encoding="utf-8"),
        ...
        check(
            "target-gate-red-exit-2",
            red.returncode == 2,
            f"exit {red.returncode}: {red.stderr}",
        )
        check(
            "target-gate-red-receipt",
            (tgt_red / RECEIPT_REL).is_file()
            and '"root_coupling_gate": "fail"'
            in (tgt_red / RECEIPT_REL).read_text(encoding="utf-8"),
        ...
        check(
            "home-residue-refused",
            residue.returncode == 64,
            f"exit {residue.returncode}: {residue.stderr}",
        )
        check(
            "home-residue-named",
            "foreign.py" in residue.stderr,
            f"stderr does not name the leaking file: {residue.stderr!r}",
        )
        subprocess.run(
            ["git", "-C", str(src), "rm", "-q", "--cached", "code/foreign.py"],
            check=True,
        )
        ...
        check(
            "absolute-symlink-refused",
            abslink.returncode == 64,
            f"exit {abslink.returncode}",
        )
        subprocess.run(
            ["git", "-C", str(src), "rm", "-q", "--cached", "code/abs_link"], check=True
        )
        ...
        check(
            "abs-path-manifest-refused",
            absres.returncode == 64,
            f"exit {absres.returncode}",
        )
