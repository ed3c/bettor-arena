...
export function loadManaged(target: string): ManagedManifest | null {
  ...
  if (value.schema !== MANAGED_SCHEMA || !Array.isArray(value.paths)) throw new ProjectError(`${path}: invalid managed manifest`);
...
function managedMap(managed: ManagedManifest | null): Map<string, { sha256: string; mode: number }> {
  ...
  for (const item of managed.paths) {
    ...
    if (result.has(path) || typeof item.sha256 !== "string" || typeof item.mode !== "number") throw new ProjectError(`managed manifest path entry drifted: ${path}`);
...
export function planProject(source: string, target: string, requirementsPath: string): ProjectPlan {
  ...
  ensureGitRepository(source); ensureGitRepository(target);
  ...
  if (managed && managed.project !== requirements.id) throw new ProjectError(`target is managed for another project: ${managed.project}`);
  ...
  return withDigest({
    ...
    files,
    embedded: requirements.mode === "embedded-core" ? { path: EMBEDDED_PATH, commit: requirements.release.commit } : null,
  }) as ProjectPlan;
}
...
function verifyPlan(plan: ProjectPlan): void {
  ...
  for (const item of plan.files) {
    ...
    if (shaOrNull(before) !== item.before_sha256) throw new ProjectError(`project plan before payload drifted: ${item.path}`);
    if (item.action === "write") {
      const after = fromBase64(item.after_base64);
      if (!after || item.after_sha256 !== sha256Bytes(after) || item.mode === null) throw new ProjectError(`project plan write payload drifted: ${item.path}`);
    } else if (item.action === "delete") {
      if (item.after_base64 !== null || item.after_sha256 !== null || item.mode !== null) throw new ProjectError(`project plan delete payload drifted: ${item.path}`);
...
function cloneEmbedded(source: string, target: string, commit: string): void {
  ...
  try {
    ...
    if (runGit(vendor, ["rev-parse", "HEAD"]) !== commit) throw new ProjectError("embedded clone resolved the wrong commit");
    if (runGit(vendor, ["status", "--porcelain", "--untracked-files=all"])) throw new ProjectError("embedded clone is dirty after checkout");
  } catch (error) { rmSync(vendor, { recursive: true, force: true }); throw error }
}
...
function embeddedState(target: string): string | null {
  ...
  if (runGit(vendor, ["status", "--porcelain", "--untracked-files=all"])) throw new ProjectError(`embedded clone is dirty: ${EMBEDDED_PATH}`);
...
function buildManaged(plan: ProjectPlan): ManagedManifest {
  return withDigest({
    ...
    source: plan.release,
    paths: plan.files.filter((item) => item.action === "write").map((item) => ({ path: item.path, sha256: item.after_sha256!, mode: item.mode! })).sort((a, b) => a.path.localeCompare(b.path)),
...
export function applyProject(source: string, target: string, planPath: string, receiptPath?: string): string {
  ...
  verifyPlan(plan); requireCommit(source, plan.release.commit);
  ...
  if (shaOrNull(currentManaged) !== plan.previous_managed_sha256) throw new ProjectError("managed manifest changed since plan");
  for (const item of plan.files) if (shaOrNull(currentBytes(targetPath(target, item.path))) !== item.before_sha256) throw new ProjectError(`target changed since plan: ${item.path}`);
  if (plan.embedded && embeddedState(target) !== null) throw new ProjectError(`embedded path already exists: ${EMBEDDED_PATH}`);

  const managedBytes = Buffer.from(`${JSON.stringify(buildManaged(plan), null, 2)}\n`);
  const finalReceiptPath = receiptPath ? resolve(receiptPath) : targetPath(target, `${RECEIPT_DIR}/apply-${plan.content_sha256.slice(0, 12)}.json`);
  ...
  try {
    if (plan.embedded) { cloneEmbedded(source, target, plan.embedded.commit); embeddedCreated = true }
    for (const item of plan.files) {
    ...
    for (const item of [...applied].reverse()) restore(targetPath(target, item.path), fromBase64(item.before_base64), item.mode);
...
export function verifyProject(target: string): ManagedManifest {
  target = resolve(target); ensureGitRepository(target);
  const managed = loadManaged(target);
  if (!managed) throw new ProjectError("project is not managed by bettor-arena");
  for (const item of managed.paths) {
    const path = targetPath(target, item.path);
    const bytes = currentBytes(path);
    if (!bytes || sha256Bytes(bytes) !== item.sha256) throw new ProjectError(`managed projection missing or modified: ${item.path}`);
    if ((statSync(path).mode & 0o777) !== item.mode) throw new ProjectError(`managed projection mode drifted: ${item.path}`);
  }
  if (managed.embedded) {
    const head = embeddedState(target);
    if (head !== managed.embedded.commit) throw new ProjectError(`embedded release drifted: expected ${managed.embedded.commit}, got ${head}`);
  }
  return managed;
}
...
export function rollbackProject(target: string, receiptPath: string): void {
  ...
  if (receipt.schema !== RECEIPT_SCHEMA || !Array.isArray(receipt.files)) throw new ProjectError("invalid apply receipt");
  verifyDigest(receipt as unknown as Record<string, unknown>, "apply receipt");
  const managedPath = targetPath(target, MANAGED_PATH);
  const managedBytes = currentBytes(managedPath);
  if (!managedBytes || sha256Bytes(managedBytes) !== receipt.managed_after_sha256) throw new ProjectError("managed manifest changed after apply; rollback refused");
  for (const item of receipt.files) if (shaOrNull(currentBytes(targetPath(target, item.path))) !== item.after_sha256) throw new ProjectError(`rollback target changed after apply: ${item.path}`);
  if (receipt.embedded_created && embeddedState(target) !== receipt.embedded_created.commit) throw new ProjectError("embedded clone changed after apply; rollback refused");
  for (const item of [...receipt.files].reverse()) restore(targetPath(target, item.path), fromBase64(item.before_base64), item.mode);
