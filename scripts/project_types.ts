...
export interface ModuleSelection { id: string; components: string[] }
...
export interface ResolvedComposition {
  modules: Array<{
    id: string; interface_version: string; manifest_sha256: string;
    components: string[]; provides: string[]; roots: string[];
  }>;
...
export class ProjectError extends Error {
  constructor(message: string) { super(message); this.name = "ProjectError" }
}
...
export function sha256Json(value: unknown): string { return sha256Bytes(canonical(value)) }
...
export function verifyDigest(value: Record<string, unknown>, label: string): void {
  ...
  const unsigned = { ...value }; delete unsigned.content_sha256;
  if (typeof claimed !== "string" || claimed !== sha256Json(unsigned)) throw new ProjectError(`${label}: content_sha256 mismatch`);
}
export function readJson<T = Record<string, unknown>>(path: string): T {
  try {
    const parsed = JSON.parse(readFileSync(path, "utf8"));
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) throw new ProjectError(`${path}: JSON root must be an object`);
    return parsed as T;
  } catch (error) {
...
export function atomicWrite(path: string, content: Buffer, mode: number): void {
  ...
  writeFileSync(temp, content, { mode }); chmodSync(temp, mode); renameSync(temp, path);
}
...
export function runGit(root: string, args: string[], allow = new Set([0])): string {
  const result = spawnSync("git", ["-c", "core.hooksPath=/dev/null", "-c", "core.fsmonitor=false", "-C", root, ...args], {
    encoding: "utf8", maxBuffer: 8 * 1024 * 1024,
  });
  const code = result.status ?? 64;
  if (!allow.has(code)) throw new ProjectError((result.stderr || result.stdout || `git ${args.join(" ")} failed`).trim());
...
export function gitHeadOrNull(root: string): string | null {
  const result = spawnSync("git", ["-c", "core.hooksPath=/dev/null", "-c", "core.fsmonitor=false", "-C", root, "rev-parse", "HEAD"], { encoding: "utf8" });
  return result.status === 0 ? result.stdout.trim() : null;
}
export function gitTree(root: string, commit: string): string { return runGit(root, ["rev-parse", `${commit}^{tree}`]) }
...
export function targetPath(target: string, managedPath: string): string {
  ...
  if (rel.startsWith(`..${sep}`) || rel === ".." || isAbsolute(rel)) throw new ProjectError(`managed path escapes target: ${managedPath}`);
  return absolute;
}
...
export function shaOrNull(value: Buffer | null): string | null { return value ? sha256Bytes(value) : null }
export function toBase64(value: Buffer | null): string | null { return value ? value.toString("base64") : null }
export function fromBase64(value: string | null): Buffer | null { return value === null ? null : Buffer.from(value, "base64") }
export function assertClosedObject(value: unknown, fields: string[], label: string): asserts value is Record<string, unknown> {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new ProjectError(`${label}: object required`);
  if (canonical(Object.keys(value as Record<string, unknown>).sort()) !== canonical([...fields].sort())) throw new ProjectError(`${label}: fields drifted`);
}
...
export function validateRequirements(value: unknown, label = "project requirements"): ProjectRequirements {
  ...
  if (value.mode !== "remote-consumer" && value.mode !== "embedded-core") throw new ProjectError(`${label}: unsupported mode`);
  assertClosedObject(value.release, ["repository", "commit"], `${label}.release`);
  if (typeof value.release.repository !== "string" || !value.release.repository.trim()) throw new ProjectError(`${label}: release.repository is required`);
  const repository = value.release.repository;
  if (isAbsolute(repository) || repository.startsWith("~/") || repository.startsWith("file://") || repository.includes("/Users/")) {
    throw new ProjectError(`${label}: release.repository must be a portable Git remote identity`);
  }
  if (typeof value.release.commit !== "string" || !SHA40.test(value.release.commit)) throw new ProjectError(`${label}: release.commit must be exactly 40 lowercase hex`);
  if (value.preset !== null && (typeof value.preset !== "string" || !ID.test(value.preset))) throw new ProjectError(`${label}: invalid preset`);
  ...
  const modules = value.modules.map((entry, index) => {
    ...
    if (typeof entry.id !== "string" || !ID.test(entry.id) || seen.has(entry.id)) throw new ProjectError(`${label}: invalid or duplicate module ${String(entry.id)}`);
    seen.add(entry.id);
    return { id: entry.id, components: stringArray(entry.components, `${label}:${entry.id}.components`) };
  });
  return { schema: REQUIREMENTS_SCHEMA, id: value.id, mode: value.mode, release: { repository, commit: value.release.commit }, preset: value.preset, modules };
}
