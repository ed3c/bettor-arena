...
import { applyProject, copyRequirements, planProject, rollbackProject, verifyProject } from "./project_transaction.ts";
...
function isProjectCommand(value: string): value is NonNullable<Options["command"]> {
  return value === "plan" || value === "apply" || value === "verify" || value === "rollback";
}
...
function parseArgs(argv: string[]): Options {
  ...
  while (rest.length) {
    ...
    if (!["source", "requirements", "target", "output", "plan", "receipt"].includes(key))
      throw new ProjectError(`unknown argument: ${token}`);
  ...
  if (options.command === "plan" && (!options.requirements || !options.output))
    throw new ProjectError("plan: --requirements and --output are required");
...
export function selftest(source: string): number {
  ...
  if (runGit(source, ["status", "--porcelain", "--untracked-files=all"]))
    throw new ProjectError("selftest requires a clean source checkout");
  ...
  try {
    ...
    if (remotePlan.files.length < 8 || remotePlan.embedded !== null)
      throw new ProjectError("selftest remote plan has incomplete projections");
    ...
    if (existsSync(agentPath) || existsSync(join(remoteTarget, MANAGED_PATH)))
      throw new ProjectError("selftest rollback left managed projections");
    ...
    const alteredUnsigned = {
      ...
      paths: [
        ...managed.paths,
        { path: orphanRelative, sha256: sha256Bytes(readFileSync(join(remoteTarget, orphanRelative))), mode: 0o644 },
      ],
    ...
    if (!cleanupPlan.files.some((item) => item.path === orphanRelative && item.action === "delete"))
      throw new ProjectError("selftest orphan projection was not named for deletion");
    ...
    if (lstatSync(vendor).isSymbolicLink() || runGit(vendor, ["rev-parse", "HEAD"]) !== commit)
      throw new ProjectError("selftest embedded release is not an independent exact clone");
  ...
  try {
    ...
    if (options.command === "plan") {
      ...
      output({
        ...
        files: plan.files.map(({ path, action }) => ({ path, action })),
      });
    } else if (options.command === "apply") {
      const receipt = applyProject(
        resolve(options.source),
        target,
        resolve(options.plan!),
        options.receipt ? resolve(options.receipt) : undefined,
      );
    ...
    if (error instanceof ProjectError) {
      console.error(`PROJECT-BOOTSTRAP-RED ${error.message}`);
      return 2;
    }
