...
function validateModule(value: unknown, path: string): ModuleManifest {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new ProjectError(`${path}: module object required`);
  ...
  if (!module.components || typeof module.components !== "object" || Array.isArray(module.components)) throw new ProjectError(`${path}: components object required`);
  for (const [name, component] of Object.entries(module.components)) {
    if (!ID.test(name) || !component || typeof component.required !== "boolean") throw new ProjectError(`${path}: invalid component ${name}`);
  ...
  if (!module.skills || !module.external_policy || typeof module.interface_version !== "string") throw new ProjectError(`${path}: incomplete module contract`);
...
export function loadModules(source: string): Map<string, { manifest: ModuleManifest; path: string }> {
  ...
  for (const name of readdirSync(root).sort()) {
    ...
    if (manifest.id !== name || modules.has(manifest.id)) throw new ProjectError(`${path}: module directory/id mismatch`);
...
function mergeSelections(base: ModuleSelection[], extra: ModuleSelection[]): ModuleSelection[] {
  ...
  return [...merged.entries()].sort(([a], [b]) => a.localeCompare(b)).map(([id, components]) => ({ id, components: [...components].sort() }));
}
...
export function selectionFromRequirements(source: string, requirements: ProjectRequirements): ModuleSelection[] {
  ...
  if (requirements.preset) {
    ...
    if (value.schema !== "bettor-arena/preset/v1" || value.id !== requirements.preset || !Array.isArray(value.modules)) throw new ProjectError(`${path}: invalid preset`);
...
export function resolveComposition(source: string, selection: ModuleSelection[], modules = loadModules(source)): ResolvedComposition {
  const providers = new Map<string, string>();
  for (const [id, { manifest }] of modules) {
    for (const capability of manifest.provides) {
      if (providers.has(capability)) throw new ProjectError(`capability has multiple providers: ${capability}: ${providers.get(capability)}, ${id}`);
  ...
  for (const entry of selection) {
    ...
    requested.set(entry.id, new Set(entry.components)); queue.push(entry.id);
  }
  ...
  for (const id of [...selected].sort()) {
    ...
    const components = requested.get(id) ?? new Set(Object.entries(manifest.components).filter(([, value]) => value.required).map(([name]) => name));
    ...
    const omitted = Object.entries(manifest.components).filter(([, value]) => value.required).map(([name]) => name).filter((name) => !components.has(name));
...
export function selectedSkillRequirements(project: string, composition: ResolvedComposition, modules: Map<string, { manifest: ModuleManifest; path: string }>): Record<string, unknown> {
...
export function buildConsumerLock(source: string, requirements: ProjectRequirements, composition: ResolvedComposition, skills: Record<string, unknown>): Record<string, unknown> & { content_sha256: string } {
...
function launcher(mode: ProjectRequirements["mode"], commit: string): string {
  const rootExpression = mode === "remote-consumer"
...
export function generatedFiles(requirements: ProjectRequirements, lock: Record<string, unknown>, skills: Record<string, unknown>): Map<string, { content: Buffer; mode: number }> {
...
`;
  const mcp = { mcpServers: { "bettor-arena": { type: "stdio", command: "sh", args: ["-c", 'repo_root="$(git rev-parse --show-toplevel)" || exit 64; exec "$repo_root/.arena/bin/bettor-mcp"'], env: {} } } };
...
`;
  const values = new Map<string, { content: Buffer; mode: number }>([
    ...
    [".arena/consumer.requirements.json", { content: Buffer.from(`${JSON.stringify(requirements, null, 2)}\n`), mode: 0o644 }],
    [".arena/consumer.lock.json", { content: Buffer.from(`${JSON.stringify(lock, null, 2)}\n`), mode: 0o644 }],
    [".agents/shared-skills.requirements.json", { content: Buffer.from(`${JSON.stringify(skills, null, 2)}\n`), mode: 0o644 }],
    [".mcp.json", { content: Buffer.from(`${JSON.stringify(mcp, null, 2)}\n`), mode: 0o644 }],
    [".codex/config.toml", { content: Buffer.from(codex), mode: 0o644 }],
  ]);
  const forbidden = ["ANTHROPIC" + "_API_KEY=", "OPENAI" + "_API_KEY=", "E2B" + "_API_KEY=", "CODEX" + "_API_KEY=", "/Users/", "~/"];
  for (const [path, value] of values) {
    const text = value.content.toString("utf8");
    for (const marker of forbidden) if (text.includes(marker)) throw new ProjectError(`generated projection contains forbidden material ${marker}: ${path}`);
