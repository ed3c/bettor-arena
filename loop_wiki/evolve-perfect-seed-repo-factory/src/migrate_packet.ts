...
import { SENTINEL_SOURCE_REF } from "./contracts";
...
const migrated = {
  ...
  // Pre-source-refs packets carry the marked sentinel ref: validate accepts it,
  // but route-result and lineage record refs_status:"sentinel" so the packet never fakes an anchor.
  source_refs: legacy.source_refs ?? [SENTINEL_SOURCE_REF],
