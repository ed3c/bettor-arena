...
- Reach taxonomy: every gate above is STATIC or SANDBOX per
  `modules/eight-base-laws.md`; the PROD reach (Production Use axis) is not yet
  wired, so validated→seed stays a human gate by criterion, not by debt.
- Refutation ledger rule: the trajectory below is append-only; a row later
  judged wrong gets a `note:` naming why it was believed, never a rewrite.
...
| 8    | main-session | GREEN  | global-law landing `modules/eight-base-laws.md` (skills-shared \_global claude laws × invariant-reach-graph reach/refutation) wired into read order, passive context, and verify structural gates; verify 29 factory tests + 2 generated tests/37 assertions pass; planted defects go red at the right gate (missing file exit 2, diluted settled criterion exit 2); selftest hollow control passes |
