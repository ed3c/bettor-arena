...
CLASSIFICATIONS = {
    "WHITE_BOX_REFERENCE",
    "SOURCE_VISIBLE_GRAY_MODEL",
    "GRAY_BOX",
    "EXPERIMENTAL_GRAY_BOX",
}
...
def validate_adapter(
    value: Any, label: str = "adapter", *, allow_fixture: bool = False
) -> dict[str, Any]:
    ...
    for index, item in enumerate(argv):
        ...
        if "\n" in item or "\r" in item or "\0" in item:
            raise ContractError(
                f"{label}.version_argv[{index}] contains a control character"
            )
    ...
    if source is not None:
        source = exact_object(
            source, {"repository", "ref", "digest"}, f"{label}.source_identity"
        )
        if not REPO.fullmatch(
            bounded(source["repository"], f"{label}.source_identity.repository", 256)
        ):
    ...
    if (
        adapter["trace_ceiling"] == "SOURCE_VERIFIED_INTERNAL"
        and source is None
        and not allow_fixture
    ):
        raise ContractError(
            f"{label} claims source-verified internals without a pinned source identity"
        )
    caps = exact_object(
    ...
    if (
        adapter["classification"] in {"GRAY_BOX", "EXPERIMENTAL_GRAY_BOX"}
        and adapter["trace_ceiling"] == "SOURCE_VERIFIED_INTERNAL"
    ):
        raise ContractError(
            f"{label} gray-box adapter cannot claim source-verified internal trace"
        )
...
def validate_registry(
    root: Path, value: Any | None = None
) -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    registry_path = root / "adapters" / "registry.json"
    registry = exact_object(
        load_json(registry_path) if value is None else value,
        REGISTRY_KEYS,
        "adapter registry",
    )
    ...
    if names != expected:
        raise ContractError(
            f"adapter registry must preserve exact six-host order: {expected}"
        )
    if (
        registry["live_matrix_state"] != "NOT_EXERCISED"
        or registry["human_admit"] is not False
    ):
        raise ContractError("adapter registry fabricated live matrix or Human Admit")
    descriptors: dict[str, dict[str, Any]] = {}
    for name in names:
        descriptor = validate_adapter(
            load_json(root / "adapters" / f"{name}.json"), f"adapter[{name}]"
        )
    ...
    if descriptors["grok-build"]["source_identity"] is None:
        raise ContractError(
            "Grok Build white-box reference lacks the reviewed source pin"
        )
    if descriptors["ante"]["implementation_state"] != "NOT_IMPLEMENTED":
        raise ContractError(
            "Ante must remain NOT_IMPLEMENTED until its runtime adapter is physically admitted"
        )
    if any(
        item["implementation_state"] == "IMPLEMENTED" for item in descriptors.values()
    ):
        raise ContractError(
            "live host adapter support was fabricated in the contract-only registry"
        )
...
def validate_request(
    ...
    if adapters is not None:
        if (
            adapter_id not in adapters
            or adapters[adapter_id]["host_id"] != request["host_id"]
        ):
            raise ContractError(f"{label} adapter/host identity mismatch")
    skill = exact_object(
        request["skill"], {"name", "digest", "source_ref"}, f"{label}.skill"
    )
    ...
    context = exact_object(
        request["context"], {"digest", "entry_files"}, f"{label}.context"
    )
    sha_ref(context["digest"], f"{label}.context.digest")
    if (
        not isinstance(context["entry_files"], list)
        or not context["entry_files"]
        or len(context["entry_files"]) != len(set(context["entry_files"]))
    ):
    ...
    workspace = exact_object(
        request["workspace"],
        {"lease_id", "writable_paths", "read_only_paths", "cleanup"},
        f"{label}.workspace",
    )
    ...
    for key in ("writable_paths", "read_only_paths"):
        ...
        for index, item in enumerate(values):
            ...
            if normalized in all_paths:
                raise ContractError(
                    f"{label}.workspace path is both writable and read-only: {normalized}"
                )
    ...
    if (
        type(policy["timeout_ms"]) is not int
        or not 1000 <= policy["timeout_ms"] <= 3_600_000
    ):
        raise ContractError(f"{label}.policy.timeout_ms invalid")
    if (
        type(policy["max_output_bytes"]) is not int
        or not 1024 <= policy["max_output_bytes"] <= 104_857_600
    ):
        raise ContractError(f"{label}.policy.max_output_bytes invalid")
    if (
        type(policy["max_processes"]) is not int
        or not 1 <= policy["max_processes"] <= 64
    ):
    ...
    for item in envs:
        if (
            ...
            or not ENV.fullmatch(item)
            or SECRET_KEYS.search(item)
        ):
            raise ContractError(
                f"{label}.policy.env_allowlist contains unsafe key: {item!r}"
            )
    task = exact_object(
        request["task"], {"prompt_ref", "mode", "expected_artifacts"}, f"{label}.task"
    )
...
def validate_event(
    ...
    if (
        ...
        or event["host_id"] != request["host_id"]
    ):
    ...
    ceiling_rank = {
        "PROCESS_ONLY": 0,
        "TOOL_OBSERVED": 1,
        "SOURCE_VERIFIED_INTERNAL": 2,
    }[ceiling]
    if ranks[event["visibility"]] > ceiling_rank:
        raise ContractError(f"{label} exceeds adapter trace ceiling")
    if (
        descriptor["classification"] in {"GRAY_BOX", "EXPERIMENTAL_GRAY_BOX"}
        and event["visibility"] == "SOURCE_VERIFIED_INTERNAL"
    ):
        raise ContractError(f"{label} fabricated gray-box internals")
    payload = exact_object(
        event["payload"],
        {"message", "exit_code", "tool", "artifact_ref", "cleanup_state"},
        f"{label}.payload",
    )
...
def validate_receipt(
    ...
    if (
        adapter["adapter_id"] != descriptor["adapter_id"]
        or adapter["host_id"] != descriptor["host_id"]
    ):
    ...
    process = exact_object(
        receipt["process"],
        {"exit_code", "timed_out", "cancelled", "process_group_killed"},
        f"{label}.process",
    )
    ...
    if any(
        ...
        for key in ("timed_out", "cancelled", "process_group_killed")
    ):
        raise ContractError(f"{label}.process flags invalid")
    trace = exact_object(
        receipt["trace"],
        {"completeness", "events_digest", "event_count", "opaque_segments"},
        f"{label}.trace",
    )
    ...
    cleanup = exact_object(
        receipt["cleanup"], {"state", "residue_paths"}, f"{label}.cleanup"
    )
    if cleanup["state"] not in {"PASS", "FAIL", "NOT_RUN"} or not isinstance(
        cleanup["residue_paths"], list
    ):
    ...
    authority = exact_object(
        ...
        {
            "wrote_loopx_state",
            "submitted_gate_verdict",
            "performed_human_admit",
            "promoted_release",
            "wrote_durable_memory",
        },
        f"{label}.authority",
    )
    ...
    if receipt["status"] == "PASS":
        if (
            receipt["executed"] is not True
            or process["exit_code"] != 0
            or cleanup["state"] != "PASS"
        ):
    ...
    if receipt["status"] in {"NOT_EXERCISED", "ABSENT", "SKIPPED_BY_POLICY"}:
        if receipt["executed"] is not False or process["exit_code"] is not None:
            raise ContractError(
                f"{label} non-execution state contains execution evidence"
            )
