...
def validate_manifest(root: Path, override: Any | None = None) -> dict[str, Any]:
    manifest = exact(
        load_json(root / "contracts" / "manifest.json")
        if override is None
        else override,
        MANIFEST_KEYS,
        "gateway manifest",
    ...
    if (
        manifest["live_matrix_state"] != "NOT_EXERCISED"
...
def validate_fixtures(
    ...
    request = validate_request(
        load_json(fixture / "request.json"), descriptors, "fixture request"
    )
    descriptor = descriptors[request["adapter_id"]]
    receipt = validate_receipt(
        load_json(fixture / "receipt.json"), request, descriptor, "fixture receipt"
    )
    if receipt["status"] != "NOT_EXERCISED":
        raise ContractError("fixture receipt must preserve NOT_EXERCISED")
    event = validate_event(
        load_json(fixture / "event.json"), request, descriptor, 0, "fixture event"
    )
...
def selftest(root: Path) -> None:
    ...
    raw = copy.deepcopy(bad_registry)
    ...
    raw = copy.deepcopy(bad_descriptor)
    raw.pop("content_digest")
    ...
    with tempfile.TemporaryDirectory(prefix="loopx-worker-registry-mutation.") as temp:
        ...
        expect_red(
            lambda: validate_registry(mutated_root), "fabricated implemented adapter"
        )
    mutations += 1
    ...
    for label, mutate in [
        ...
        (
            "path traversal",
            lambda x: x["workspace"]["writable_paths"].__setitem__(0, "../escape"),
        ),
        ("secret env", lambda x: x["policy"]["env_allowlist"].append("API_KEY")),
        (
            "raw authority field",
            lambda x: x["task"].__setitem__("gate_verdict", "PASS"),
        ),
        (
            "bad context digest",
            lambda x: x["context"].__setitem__("digest", "sha256:" + "0" * 63),
        ),
    ]:
    ...
    raw = copy.deepcopy(fake_pass)
    raw.pop("content_digest")
    fake_pass["content_digest"] = digest(raw)
    expect_red(
        lambda: validate_receipt(fake_pass, request, descriptor),
    ...
    expect_red(
        lambda: validate_receipt(authority, request, descriptor), "Worker state write"
    )
    mutations += 1
    ...
    raw = copy.deepcopy(cleanup)
    raw.pop("content_digest")
    cleanup["content_digest"] = digest(raw)
    expect_red(
        lambda: validate_receipt(cleanup, request, descriptor),
    ...
    gap["sequence"] = 2
    raw = copy.deepcopy(gap)
    raw.pop("content_digest")
    gap["content_digest"] = digest(raw)
    expect_red(
        lambda: validate_event(gap, request, descriptor, 0), "event sequence gap"
    )
    mutations += 1

    internal = copy.deepcopy(event)
    internal["visibility"] = "SOURCE_VERIFIED_INTERNAL"
    raw = copy.deepcopy(internal)
    raw.pop("content_digest")
    internal["content_digest"] = digest(raw)
    expect_red(
        lambda: validate_event(internal, request, descriptor, 0),
        "gray/process-only internal event",
    )
    mutations += 1
    ...
    raw = copy.deepcopy(private)
    raw.pop("content_digest")
    private["content_digest"] = digest(raw)
    expect_red(
        lambda: validate_event(private, request, descriptor, 0), "private reasoning"
    )
    mutations += 1

    secret_value = copy.deepcopy(event)
    secret_value["payload"]["message"] = "Bearer abcdefghijklmnopqrstuvwxyz123456"
    raw = copy.deepcopy(secret_value)
    raw.pop("content_digest")
    secret_value["content_digest"] = digest(raw)
    expect_red(
        lambda: validate_event(secret_value, request, descriptor, 0),
        "secret-shaped event",
    )
    mutations += 1

    print(
        f"loopx-worker-gateway selftest PASS: 1 positive, 1 not-exercised, {mutations} mutations"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.selftest:
            ...
            states = {
                name: descriptor["implementation_state"]
