#!/usr/bin/env python3
"""Shared exits, errors and deterministic encoding for LoopX Strategy + HITL.

Exit codes follow the repository contract: 0 ok, 2 a checked invariant
disagrees, 64 the input or invocation is unusable. The distinction is
load-bearing here -- an absent decision file and a rejected decision must not
...
# Deliberately narrow: a timestamp this validator cannot order is a timestamp an
# expiry check cannot use.
...
def canonical_bytes(value: Any) -> bytes:
    """Encode deterministically so digests do not depend on key order."""
...
def exact_object(value: Any, keys: set[str], label: str) -> dict[str, Any]:
    """Fail closed on both missing and unexpected keys.

    A packet that gained a field is as much a contract break as one that lost
    it: the extra field means some producer believes in a channel this consumer
    does not read.
    """
