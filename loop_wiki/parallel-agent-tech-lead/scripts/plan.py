...
UPSTREAM_SCHEMA = (
    "skills/git-town-stacked-pr-worker/references/FAN_OUT_CONTRACT.schema.json"
)
...
def infer_mode(workers: list[dict[str, Any]]) -> str:
    ...
    has_non_competitor_parallel = any(
        worker.get("role") == "sibling" for worker in workers
    )
    has_dependency = any(
        worker.get("depends_on")
        for worker in workers
        if worker.get("role") != "convergence"
    )
...
def ancestors(
    worker_id: str, workers: dict[str, dict[str, Any]], seen: set[str] | None = None
) -> set[str]:
...
def validate(plan: Any) -> dict[str, Any]:
    ...
    require(
        ...
        "upstream fan-out contract commit drift",
    )
    require(
        upstream.get("schema_path") == UPSTREAM_SCHEMA,
        "upstream fan-out schema path drift",
    )
    require(
        upstream.get("schema_blob") == UPSTREAM_SCHEMA_BLOB,
        "upstream fan-out schema blob drift",
    )
    ...
    require(
        is_sha(base.get("commit"), 40) and is_sha(base.get("tree"), 40),
        "base commit/tree invalid",
        invalid=True,
    )
    ...
    require(
        isinstance(funnel.get("receipt"), str) and funnel["receipt"],
        "CONTEXT_FUNNEL_STATE_LAUNDERED",
    )
    require(
        is_sha(funnel.get("digest"), 64), "context funnel digest invalid", invalid=True
    )
    ...
    for key in (
        ...
        "max_processes_per_worker",
        "max_output_bytes_per_worker",
    ):
        value = budgets.get(key)
        require(
            isinstance(value, int) and not isinstance(value, bool) and value >= 0,
            f"budgets.{key} invalid",
            invalid=True,
        )
    require(
        budgets["max_workers"] > 0
        and budgets["max_tokens_per_worker"] > 0
        and budgets["max_wall_clock_seconds"] > 0
        and budgets["max_processes_per_worker"] > 0
        and budgets["max_output_bytes_per_worker"] > 0,
        "zero worker budget",
    )
    require(
        isinstance(budgets.get("circuit_breakers"), list)
        and budgets["circuit_breakers"],
        "circuit_breakers missing",
        invalid=True,
    )
    ...
    require(
        isinstance(immutable, list) and immutable,
        "immutable acceptance paths missing",
        invalid=True,
    )
    require(
        isinstance(oracles, list) and len(oracles) >= 2,
        "at least two acceptance oracles required",
        invalid=True,
    )
    ...
    require(
        isinstance(worker_list, list) and worker_list, "workers missing", invalid=True
    )
    ...
    for index, worker in enumerate(worker_list):
        ...
        require(
            isinstance(worker_id, str) and worker_id and worker_id not in workers,
            "worker id invalid/duplicate",
            invalid=True,
        )
        ...
        require(
            isinstance(branch, str) and branch and branch not in branches,
            f"{worker_id}: branch invalid/duplicate",
            invalid=True,
        )
        branches.add(branch)
        require(
            worker.get("parent") == base.get("branch")
            or worker.get("parent") in branches
            or isinstance(worker.get("parent"), str),
            f"{worker_id}: parent missing",
            invalid=True,
        )
        deps = worker.get("depends_on")
        require(
            isinstance(deps, list), f"{worker_id}: depends_on invalid", invalid=True
        )
        writable = worker.get("writable_paths")
        require(
            isinstance(writable, list) and writable,
            f"{worker_id}: writable_paths missing",
            invalid=True,
        )
        ...
        require(
            ...
            f"{worker_id}: read_only_paths invalid",
            invalid=True,
        )
        worker["read_only_paths"] = [safe_path(path) for path in read_only]
        require(
            worker.get("context_digest") == context["digest"],
            f"{worker_id}: CONTEXT_DIGEST_MISMATCH",
        )
        require(
            isinstance(worker.get("token_budget"), int)
            and 0 < worker["token_budget"] <= budgets["max_tokens_per_worker"],
            f"{worker_id}: WORKER_BUDGET_OVERFLOW",
        )
        require(
            isinstance(worker.get("timeout_seconds"), int)
            and 0 < worker["timeout_seconds"] <= budgets["max_wall_clock_seconds"],
            f"{worker_id}: timeout overflow",
        )
        require(
            isinstance(worker.get("process_budget"), int)
            and 0 < worker["process_budget"] <= budgets["max_processes_per_worker"],
            f"{worker_id}: process overflow",
        )
        require(
            isinstance(worker.get("output_byte_budget"), int)
            and 0
            < worker["output_byte_budget"]
            <= budgets["max_output_bytes_per_worker"],
            f"{worker_id}: output overflow",
        )
        for lease in worker["writable_paths"]:
            for protected in immutable:
                require(
                    not path_overlap(lease, protected),
                    f"{worker_id}: ACCEPTANCE_TEST_MUTATED",
                )

    for worker in worker_list:
        for dependency in worker["depends_on"]:
            require(
                dependency in workers and dependency != worker["id"],
                f"{worker['id']}: UNDECLARED_DEPENDENCY",
            )
        ...
        if worker["role"] == "child":
            ...
            require(
                worker.get("consumes_contracts") or worker.get("consumes_paths"),
                f"{worker['id']}: FAKE_LINEAR_CHILD",
            )
    ...
    for worker in competitors:
        ...
        require(
            worker.get("focus") in FOCUSES and worker["focus"] not in competitor_focus,
            f"{worker['id']}: MISSING_BRANCH_FOCUS",
        )
        competitor_focus.add(worker["focus"])

    for left_index, left in enumerate(worker_list):
        for right in worker_list[left_index + 1 :]:
            if right["id"] in ancestors(left["id"], workers) or left["id"] in ancestors(
                right["id"], workers
            ):
                continue
            ...
            for left_path in left["writable_paths"]:
                for right_path in right["writable_paths"]:
                    require(
                        not path_overlap(left_path, right_path),
                        f"PATH_OVERLAP: {left['id']} / {right['id']}",
                    )
    ...
    require(
        plan["mode"] == inferred,
        f"MODE_MISMATCH: declared {plan['mode']} inferred {inferred}",
    )
    ...
    if convergence:
        conv = convergence[0]
        leaves = {
            worker["id"] for worker in worker_list if worker["role"] != "convergence"
        }
    ...
    for key in (
        "auto_publish",
        "auto_merge",
        "auto_resolve_semantic_conflicts",
        "auto_promote",
    ):
