...
def _derive_stage(node: dict[str, Any]) -> int:
    ...
    if "OOBECeremony" in label or "CeremonyResult" in label:
        return 0
    if "protectedAppData" in label:
        return 1
    if "Hex" in label or "softKeyHex" in label:
        return 2
    if "registerV2" in label or "RegistrationHelper" in label:
        return 3
    if kind == "payload_field" or "softKeyData" in label:
        return 4
    if kind == "endpoint":
        return 5
    if kind in {"database", "vault", "external_store", "runtime_event"}:
        return 6
...
<body>
<header><div class="small">Enterprise Code Truth Graph · softKeyData vertical slice</div><h1>@@TITLE@@</h1><p>AST / LSP / SANDBOX / PROD / Agent retrieval / Human review are separate evidence lanes. No lane may silently stand in for another.</p></header>
