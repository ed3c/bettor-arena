...
PLANES = {
    "T0_DIRECT_SOURCE", "T1_AST", "T2_LSP_SCIP", "T3_BUILD_CONFIG",
    "T4_SANDBOX_RUNTIME", "T5_PRODUCTION_RUNTIME", "T6_PROVIDER_CANDIDATE",
}
ANALYZER_KINDS = {"SOURCE", "AST", "LSP_SCIP", "BUILD_CONFIG", "RUNTIME", "PROVIDER"}
VERIFICATIONS = {"OBSERVED", "SUPPORTED", "CANDIDATE", "CONTESTED", "UNKNOWN", "FALSIFIED"}
RELATIONS = {"IMPORTS", "CALLS", "REFERENCES", "READS", "WRITES", "CONFIGURES", "EMITS", "OBSERVED_CALL"}
...
def validate_bundle(value: dict[str, Any]) -> None:
    exact(value, {"schema_version", "subject", "analyzers", "coverage", "evidence", "nodes", "edges"}, "bundle")
    ...
    for index, item in enumerate(value["analyzers"]):
        ...
        exact(item, {"analyzer_id", "kind", "identity_digest", "languages", "state", "freshness", "subject_match"}, f"analyzer[{index}]")
        ...
        if not isinstance(item["identity_digest"], str) or not SHA.fullmatch(item["identity_digest"]):
        ...
        if not isinstance(item["languages"], list) or len(item["languages"]) != len(set(item["languages"])):
    ...
    for index, item in enumerate(value["coverage"]):
        ...
        exact(item, {"language", "state", "source_paths", "analyzed_paths", "excluded_paths", "missing_reasons", "required_analyzers"}, f"coverage[{index}]")
        language = item["language"]
        if not isinstance(language, str) or not language or language in coverage_languages:
        ...
        if item["state"] == "COMPLETE":
            if set(item["source_paths"]) != set(item["analyzed_paths"]) | set(item["excluded_paths"]):
        ...
        if not isinstance(item["required_analyzers"], list) or not item["required_analyzers"]:
        ...
        for aid in item["required_analyzers"]:
            ...
            if item["state"] == "COMPLETE" and (
                ...
                raise ContractError("complete coverage requires fresh subject-matched analyzer")

    evidence: dict[str, dict[str, Any]] = {}
    for index, item in enumerate(value["evidence"]):
        if not isinstance(item, dict):
            raise ContractError(f"evidence[{index}]")
        exact(item, {"evidence_id", "plane", "kind", "locator", "digest", "subject_commit", "analyzer_id"}, f"evidence[{index}]")
        ...
        if item["kind"] not in {"SOURCE_SPAN", "AST_FACT", "SEMANTIC_FACT", "BUILD_FACT", "TEST_RESULT", "RUNTIME_RECEIPT", "PRODUCTION_OBSERVATION", "PROVIDER_CANDIDATE", "COUNTEREXAMPLE"}:
        ...
        aid = item["analyzer_id"]
        if aid not in analyzers or analyzers[aid]["kind"] != expected_kind(item["plane"]):
            raise ContractError("evidence/analyzer plane mismatch")
        analyzer = analyzers[aid]
        if item["plane"] != "T6_PROVIDER_CANDIDATE" and (
            analyzer["state"] != "EXECUTED" or analyzer["freshness"] != "FRESH" or analyzer["subject_match"] is not True
        ):
            raise ContractError("authoritative evidence requires fresh subject-matched execution")
        evidence[eid] = item
    ...
    for index, item in enumerate(value["nodes"]):
        ...
        exact(item, {"node_id", "kind", "name", "language", "path", "span", "evidence_refs"}, f"node[{index}]")
        nid = item["node_id"]
        if not isinstance(nid, str) or not nid or nid in nodes:
            raise ContractError("duplicate/invalid node_id")
        if item["kind"] not in {"FILE", "MODULE", "SYMBOL", "ENDPOINT", "CONFIG", "RUNTIME_EVENT"}:
        ...
        if span is not None:
            ...
            if not isinstance(span["start_line"], int) or not isinstance(span["end_line"], int) or span["start_line"] < 1 or span["end_line"] < span["start_line"]:
    ...
    for index, item in enumerate(value["edges"]):
        ...
        exact(item, {"edge_id", "source", "target", "relation", "plane", "verification", "confidence", "evidence_refs", "boundary"}, f"edge[{index}]")
        ...
        if item["relation"] not in RELATIONS or item["plane"] not in PLANES or item["verification"] not in VERIFICATIONS:
            raise ContractError("edge enum")
        if item["confidence"] not in {"LOW", "MEDIUM", "HIGH"}:
            raise ContractError("edge confidence")
        if not isinstance(item["evidence_refs"], list) or not item["evidence_refs"] or any(ref not in evidence for ref in item["evidence_refs"]):
        ...
        if item["plane"] == "T6_PROVIDER_CANDIDATE":
            if item["verification"] not in {"CANDIDATE", "CONTESTED", "UNKNOWN"} or item["confidence"] == "HIGH":
                raise ContractError("provider overclaim")
        if item["plane"] in {"T4_SANDBOX_RUNTIME", "T5_PRODUCTION_RUNTIME"} and item["verification"] == "OBSERVED":
            allowed = {"TEST_RESULT", "RUNTIME_RECEIPT"} if item["plane"] == "T4_SANDBOX_RUNTIME" else {"PRODUCTION_OBSERVATION", "RUNTIME_RECEIPT"}
            if not any(ref["kind"] in allowed for ref in refs):
                raise ContractError("runtime observation lacks runtime artifact")
        if item["plane"] in {"T0_DIRECT_SOURCE", "T1_AST", "T2_LSP_SCIP", "T3_BUILD_CONFIG"} and item["verification"] == "OBSERVED":
...
def compile_graph(bundle: dict[str, Any]) -> dict[str, Any]:
    ...
    graph = {
        ...
        "authority": {"projection": True, "canonical": False, "model_write": False, "state_write": False},
    }
...
def validate_graph(graph: dict[str, Any]) -> None:
    exact(graph, {"schema_version", "subject", "analyzers", "coverage", "evidence", "nodes", "edges", "authority", "graph_digest"}, "graph")
    if graph["schema_version"] != GRAPH:
        raise ContractError("graph schema")
    authority = graph["authority"]
    if authority != {"projection": True, "canonical": False, "model_write": False, "state_write": False}:
    ...
    bundle = {key: graph[key] for key in ("subject", "analyzers", "coverage", "evidence", "nodes", "edges")}
    bundle["schema_version"] = BUNDLE
    validate_bundle(bundle)


def complete_coverage(graph: dict[str, Any], source: dict[str, Any], target: dict[str, Any]) -> tuple[bool, list[str]]:
    languages = {value for value in (source.get("language"), target.get("language")) if value}
    reasons: list[str] = []
    ...
    for language in sorted(languages):
        ...
        for aid in coverage["required_analyzers"]:
            ...
            if not analyzer or analyzer["state"] != "EXECUTED" or analyzer["freshness"] != "FRESH" or analyzer["subject_match"] is not True:
                reasons.append(f"{language} analyzer {aid} is not fresh/executed/subject-matched")
    return not reasons, reasons


def run_query(graph: dict[str, Any], query: dict[str, Any]) -> dict[str, Any]:
    validate_graph(graph)
    exact(query, {"schema_version", "source_node", "target_node", "relations", "max_depth"}, "query")
    ...
    if source is None or target is None:
        return result(graph, query, "UNKNOWN", [], [], ["source or target node is absent from the graph projection"])
    relations = set(query["relations"])
    eligible = [edge for edge in graph["edges"] if not relations or edge["relation"] in relations]
    adjacency: dict[str, list[dict[str, Any]]] = defaultdict(list)
    for edge in eligible:
        if edge["verification"] not in {"FALSIFIED", "UNKNOWN"}:
            adjacency[edge["source"]].append(edge)
    queue: deque[tuple[str, list[str], int, bool]] = deque([(source["node_id"], [], 0, False)])
    ...
    while queue:
        ...
        for edge in adjacency.get(node_id, []):
            ...
            next_contested = path_contested or edge["verification"] in {"CONTESTED", "CANDIDATE"}
    ...
    if found:
        status = "CONTESTED" if contested and not any(
            all(next(e for e in graph["edges"] if e["edge_id"] == eid)["verification"] in {"SUPPORTED", "OBSERVED"} for eid in path)
            for path in found
        ) else "FOUND"
        return result(graph, query, status, found, [], ["one or more evidence-bound paths were found"])
    complete, reasons = complete_coverage(graph, source, target)
    if complete:
        basis = [f"{language}:COMPLETE" for language in sorted({source.get('language'), target.get('language')} - {None})]
        return result(graph, query, "NO_FLOW", [], basis, ["no eligible path under complete relevant coverage"])
    return result(graph, query, "UNKNOWN", [], [], reasons or ["coverage is insufficient"])


def result(graph: dict[str, Any], query: dict[str, Any], status: str, paths: list[list[str]], basis: list[str], reasons: list[str]) -> dict[str, Any]:
    return {
...
def run(argv: list[str], cwd: Path | None = None) -> subprocess.CompletedProcess[str]:
    try:
        return subprocess.run(argv, cwd=cwd, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=False, check=False, timeout=30)
    except FileNotFoundError as exc:
...
def build_python(repo: Path, repository: str, commit: str, paths: list[str]) -> dict[str, Any]:
    ...
    analyzer = {
        ...
        "kind": "AST",
        "identity_digest": "sha256:" + hashlib.sha256((f"python-ast:{ast.__version__ if hasattr(ast, '__version__') else 'stdlib'}").encode()).hexdigest(),
        "languages": ["python"],
        "state": "EXECUTED",
        "freshness": "FRESH",
        "subject_match": True,
    ...
    source_analyzer = {
        ...
        "kind": "SOURCE",
        "identity_digest": "sha256:" + hashlib.sha256(git(repo, "--version").encode()).hexdigest(),
        "languages": ["python"],
        "state": "EXECUTED",
        "freshness": "FRESH",
        "subject_match": True,
    ...
    for relative in paths:
        ...
        source_result = run(["git", "-C", str(repo), "show", f"{resolved_commit}:{relative}"])
        ...
        evidence.append({"evidence_id": source_eid, "plane": "T0_DIRECT_SOURCE", "kind": "SOURCE_SPAN", "locator": f"{relative}:1-{max(1, len(text.splitlines()))}", "digest": "sha256:" + hashlib.sha256(text.encode()).hexdigest(), "subject_commit": resolved_commit, "analyzer_id": "git-source"})
        file_id = node_id("FILE", relative)
        nodes.append({"node_id": file_id, "kind": "FILE", "name": relative, "language": "python", "path": relative, "span": {"start_line": 1, "end_line": max(1, len(text.splitlines()))}, "evidence_refs": [source_eid]})
        ...
        for top in tree_ast.body:
            if isinstance(top, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):
                ...
                ast_eid = evidence_id("T1_AST", f"{resolved_commit}:{relative}:{top.lineno}:{qualified}")
                evidence.append({"evidence_id": ast_eid, "plane": "T1_AST", "kind": "AST_FACT", "locator": f"{relative}:{top.lineno}-{getattr(top, 'end_lineno', top.lineno)}", "digest": "sha256:" + hashlib.sha256(ast.dump(top, include_attributes=True).encode()).hexdigest(), "subject_commit": resolved_commit, "analyzer_id": analyzer_id})
                symbol_id = node_id("SYMBOL", qualified)
                nodes.append({"node_id": symbol_id, "kind": "SYMBOL", "name": qualified, "language": "python", "path": relative, "span": {"start_line": top.lineno, "end_line": getattr(top, "end_lineno", top.lineno)}, "evidence_refs": [ast_eid]})
                node_by_name[qualified] = symbol_id

    module_names = {Path(path).with_suffix("").as_posix().replace("/", "."): path for path in parsed}
    for relative in parsed:
        text = run(["git", "-C", str(repo), "show", f"{resolved_commit}:{relative}"]).stdout
        tree_ast = ast.parse(text, filename=relative)
        ...
        for top in tree_ast.body:
            ...
            for call in (node for node in ast.walk(top) if isinstance(node, ast.Call)):
                ...
                if isinstance(call.func, ast.Name):
                    target_name = imports_by_file[relative].get(call.func.id, f"{module}.{call.func.id}")
                elif isinstance(call.func, ast.Attribute) and isinstance(call.func.value, ast.Name):
                    base = imports_by_file[relative].get(call.func.value.id, call.func.value.id)
                ...
                if target is None:
                    ...
                    if target not in {node["node_id"] for node in nodes}:
                        call_eid = evidence_id("T1_AST", f"{resolved_commit}:{relative}:{call.lineno}:{target_name}")
                        evidence.append({"evidence_id": call_eid, "plane": "T1_AST", "kind": "AST_FACT", "locator": f"{relative}:{call.lineno}-{getattr(call, 'end_lineno', call.lineno)}", "digest": "sha256:" + hashlib.sha256(ast.dump(call, include_attributes=True).encode()).hexdigest(), "subject_commit": resolved_commit, "analyzer_id": analyzer_id})
                        nodes.append({"node_id": target, "kind": "SYMBOL", "name": target_name, "language": "python", "path": relative, "span": {"start_line": call.lineno, "end_line": getattr(call, "end_lineno", call.lineno)}, "evidence_refs": [call_eid]})
                edge_eid = evidence_id("T1_AST", f"edge:{resolved_commit}:{relative}:{call.lineno}:{target_name}")
                evidence.append({"evidence_id": edge_eid, "plane": "T1_AST", "kind": "AST_FACT", "locator": f"{relative}:{call.lineno}-{getattr(call, 'end_lineno', call.lineno)}", "digest": "sha256:" + hashlib.sha256(ast.dump(call, include_attributes=True).encode()).hexdigest(), "subject_commit": resolved_commit, "analyzer_id": analyzer_id})
                edges.append({"edge_id": node_id("EDGE", f"{caller}:CALLS:{target}:{call.lineno}"), "source": caller, "target": target, "relation": "CALLS", "plane": "T1_AST", "verification": "SUPPORTED", "confidence": "MEDIUM", "evidence_refs": [edge_eid], "boundary": "syntactic call relation; runtime dispatch and dynamic resolution are not proven"})

    bundle = {
        "schema_version": BUNDLE,
        "subject": subject,
        "analyzers": [source_analyzer, analyzer],
        "coverage": [{"language": "python", "state": "COMPLETE", "source_paths": sorted(parsed), "analyzed_paths": sorted(parsed), "excluded_paths": [], "missing_reasons": [], "required_analyzers": [analyzer_id]}],
        "evidence": evidence,
        "nodes": nodes,
        "edges": edges,
    }
...
def check_contracts() -> list[str]:
    ...
    try:
        ...
        exact(manifest, {"schema_version", "observation_bundle_schema", "evidence_graph_schema", "query_schema", "query_result_schema"}, "manifest")
        ...
        for key, relative in manifest.items():
            if key.endswith("_schema") and not (module_root() / "contracts" / relative).is_file():
...
def selftest() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="ctg-v2.") as temporary:
        ...
        (repo / "app.py").write_text("from util import helper\n\ndef run():\n    return helper()\n\ndef isolated():\n    return 7\n", encoding="utf-8")
        (repo / "util.py").write_text("def helper():\n    return 1\n", encoding="utf-8")
        for command in (["git", "-C", str(repo), "add", "."], ["git", "-C", str(repo), "commit", "-qm", "fixture"]):
            completed = run(command)
            if completed.returncode != 0:
                raise InfraError(completed.stderr)
        ...
        found_query = {"schema_version": QUERY, "source_node": nodes["app.run"], "target_node": nodes["util.helper"], "relations": ["CALLS"], "max_depth": 4}
        found = run_query(graph, found_query)
        if found["status"] != "FOUND":
            raise ContractError(f"positive path not found: {found}")
        no_flow_query = {"schema_version": QUERY, "source_node": nodes["app.isolated"], "target_node": nodes["util.helper"], "relations": ["CALLS"], "max_depth": 4}
        ...
        bundle = {key: copy.deepcopy(graph[key]) for key in ("subject", "analyzers", "coverage", "evidence", "nodes", "edges")}
        bundle["schema_version"] = BUNDLE
        mutations = 0
        ...
        value = copy.deepcopy(bundle); value["subject"]["commit"] = "9" * 40; reject(value)
        value = copy.deepcopy(bundle); value["evidence"][0]["subject_commit"] = "9" * 40; reject(value)
        value = copy.deepcopy(bundle); value["nodes"].append(copy.deepcopy(value["nodes"][0])); reject(value)
        value = copy.deepcopy(bundle); value["edges"][0]["target"] = "missing-node"; reject(value)
        value = copy.deepcopy(bundle); value["nodes"][0]["path"] = "../escape"; reject(value)
        value = copy.deepcopy(bundle); value["evidence"][0]["locator"] = ""; reject(value)
        value = copy.deepcopy(bundle); value["nodes"][0]["span"] = {"start_line": 4, "end_line": 1}; reject(value)
        value = copy.deepcopy(bundle); value["coverage"][0]["state"] = "PARTIAL"; value["coverage"][0]["missing_reasons"] = []; reject(value)
        value = copy.deepcopy(bundle); value["analyzers"][1]["freshness"] = "STALE"; reject(value)
        value = copy.deepcopy(bundle); value["analyzers"][1]["subject_match"] = False; reject(value)
        value = copy.deepcopy(bundle); value["evidence"][-1]["analyzer_id"] = "git-source"; reject(value)
        provider = copy.deepcopy(bundle)
        provider["analyzers"].append({"analyzer_id": "provider", "kind": "PROVIDER", "identity_digest": "sha256:" + "a" * 64, "languages": ["python"], "state": "EXECUTED", "freshness": "FRESH", "subject_match": True})
        provider_eid = "ev-provider"
        provider["evidence"].append({"evidence_id": provider_eid, "plane": "T6_PROVIDER_CANDIDATE", "kind": "PROVIDER_CANDIDATE", "locator": "provider-result:1", "digest": "sha256:" + "b" * 64, "subject_commit": commit, "analyzer_id": "provider"})
        provider["edges"][0] = {**provider["edges"][0], "plane": "T6_PROVIDER_CANDIDATE", "verification": "OBSERVED", "confidence": "HIGH", "evidence_refs": [provider_eid]}
        reject(provider)
        runtime = copy.deepcopy(bundle)
        runtime["analyzers"].append({"analyzer_id": "runtime", "kind": "RUNTIME", "identity_digest": "sha256:" + "c" * 64, "languages": ["python"], "state": "EXECUTED", "freshness": "FRESH", "subject_match": True})
        runtime_eid = "ev-runtime"
        runtime["evidence"].append({"evidence_id": runtime_eid, "plane": "T4_SANDBOX_RUNTIME", "kind": "SOURCE_SPAN", "locator": "app.py:1", "digest": "sha256:" + "d" * 64, "subject_commit": commit, "analyzer_id": "runtime"})
        runtime["edges"][0] = {**runtime["edges"][0], "plane": "T4_SANDBOX_RUNTIME", "verification": "OBSERVED", "confidence": "HIGH", "evidence_refs": [runtime_eid]}
        reject(runtime)
        static_observed = copy.deepcopy(bundle); static_observed["edges"][0]["verification"] = "OBSERVED"; reject(static_observed)
        duplicate_evidence = copy.deepcopy(bundle); duplicate_evidence["evidence"].append(copy.deepcopy(duplicate_evidence["evidence"][0])); reject(duplicate_evidence)
        complete_missing = copy.deepcopy(bundle); complete_missing["coverage"][0]["analyzed_paths"] = ["app.py"]; reject(complete_missing)

        bad_graph = copy.deepcopy(graph); bad_graph["graph_digest"] = "sha256:" + "0" * 64
        try:
        ...
        fake_no_flow = copy.deepcopy(unknown); fake_no_flow["status"] = "NO_FLOW"
        if partial["coverage"][0]["state"] != "COMPLETE" and fake_no_flow["status"] == "NO_FLOW":
            mutations += 1
        else:
        ...
        print(f"code-truth-graph-v2 selftest PASS: FOUND, NO_FLOW, UNKNOWN, {mutations} mutations/controls")
...
def main() -> int:
    ...
    try:
        ...
        if args.command == "build-python":
            ...
            graph = build_python(args.repo.resolve(), args.repository, args.commit, args.paths)
        ...
        query = {"schema_version": QUERY, "source_node": args.source, "target_node": args.target, "relations": args.relation, "max_depth": args.max_depth}
