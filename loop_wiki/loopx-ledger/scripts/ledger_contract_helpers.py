...
CONTRACT_KEYS = {"schema_version", "subject", "gate_definitions", "commands", "initial_state", "contract_digest"}
STORE_KEYS = {"schema_version", "subject", "contract_digest", "contract_path", "events_path", "snapshot_path", "writer_policy", "created_at", "content_digest"}
APPEND_KEYS = {"schema_version", "request_id", "expected_state_revision", "event"}
RECEIPT_KEYS = {"schema_version", "operation_id", "operation", "subject", "status", "before", "after", "artifacts", "cleanup", "details", "content_digest"}
STATE_KEYS = {"schema_version", "subject", "state_revision", "ledger_head_digest", "lifecycle", "objective", "current_todo_id", "todos", "evidence", "quota", "human_decision_refs"}
TODO_KEYS = {"todo_id", "title", "status", "depends_on", "gate_ids", "gate_results", "evidence_refs", "attempts", "last_failure_ref", "exception_ref"}
GATE_RESULT_KEYS = {"gate_id", "attempt", "verdict", "observation_ref", "evaluator_digest"}
HUMAN_KEYS = {"decision_id", "signer_ref", "signature_ref", "action", "scope", "expires_at", "rationale_ref", "required_revalidation_gate_ids", "non_waivable_acknowledged"}
...
def validate_human_decision(value: Any, label: str) -> dict[str, Any]:
    ...
    if decision["action"] not in {"RETRY_AFTER_FIX", "UPDATE_CONTRACT", "CANCEL", "SCOPED_EXCEPTION"}:
        raise ContractError(f"{label}.action is not admitted")
    scope = decision["scope"]
    if not isinstance(scope, dict) or set(scope) != {"todo_id", "gate_ids", "assertion_ids"}:
    ...
    if scope["todo_id"] is None and not scope["gate_ids"] and not scope["assertion_ids"]:
    ...
    if not isinstance(revalidation, list) or not revalidation or len(revalidation) != len(set(revalidation)):
...
def validate_gate_observation(value: Any, label: str) -> dict[str, Any]:
    keys = {"gate_id", "verdict", "observed_exit_code", "evaluator_digest", "artifact_refs"}
    ...
    if observation["observed_exit_code"] is not None and type(observation["observed_exit_code"]) is not int:
    ...
    if observation["verdict"] == "FAIL" and observation["observed_exit_code"] in {None, 0}:
    ...
    if not isinstance(observation["artifact_refs"], list) or not observation["artifact_refs"]:
...
def validate_transition(value: Any, label: str, prior_event_ids: set[str] | None = None) -> dict[str, Any]:
    ...
    for event_id in reasons:
        ...
        if prior_event_ids is not None and event_id not in prior_event_ids:
            raise ContractError(f"{label} references a future or absent event: {event_id}")
    return transition
