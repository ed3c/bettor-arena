...
def request_for(event: dict[str, Any], revision: int, request_id: str) -> dict[str, Any]:
    return {
...
def run_selftest(root: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="loopx-ledger-selftest.") as temp:
        ...
        initialize(contract_path, store, "2026-08-14T00:00:00Z", temp_root / "init.json", "selftest-init")
        for index, event_path in enumerate(event_paths):
            event = load_json(event_path)
            request_path = temp_root / f"request-{index}.json"
            atomic_write_json(request_path, request_for(event, index, f"request-{index}"))
            append_event(store, request_path, temp_root / f"append-{index}.json", f"selftest-append-{index}")
        verify_store(store, temp_root / "verify.json", "selftest-verify")
        replay_to(store, temp_root / "replay-a.json", temp_root / "replay-a-receipt.json", "selftest-replay-a")
        replay_to(store, temp_root / "replay-b.json", temp_root / "replay-b-receipt.json", "selftest-replay-b")
        if (temp_root / "replay-a.json").read_bytes() != (temp_root / "replay-b.json").read_bytes():
            raise ContractError("replay is not byte deterministic")
        expected_snapshot = fixtures / "expected-snapshot.json"
        if not expected_snapshot.is_file() or (temp_root / "replay-a.json").read_bytes() != expected_snapshot.read_bytes():
            raise ContractError("replay disagrees with the checked positive snapshot")
        duplicate_request = temp_root / "duplicate.json"
        atomic_write_json(duplicate_request, request_for(load_json(event_paths[-1]), len(event_paths), "request-duplicate"))
        receipt = append_event(store, duplicate_request, temp_root / "duplicate-receipt.json", "selftest-duplicate")
        ...
        stale_event["previous_event_digest"] = load_json(store / "snapshot.json")["ledger"]["head_digest"]
        stale_event["event_digest"] = ""
        raw = copy.deepcopy(stale_event); raw.pop("event_digest")
        ...
        expect_error(lambda: append_event(store, stale_request, temp_root / "stale-receipt.json", "selftest-stale"), "stale revision", "stale expected revision")
        mutations += 1

        collision = copy.deepcopy(load_json(event_paths[-1]))
        collision["occurred_at"] = "2026-08-14T00:00:59Z"
        raw = copy.deepcopy(collision); raw.pop("event_digest")
        collision["event_digest"] = digest(raw)
        collision_request = temp_root / "collision.json"
        atomic_write_json(collision_request, request_for(collision, len(event_paths), "request-collision"))
        expect_error(lambda: append_event(store, collision_request, temp_root / "collision-receipt.json", "selftest-collision"), "event ID collision", "collision")
        mutations += 1
        ...
        for name, mutate in [
            ...
            ("previous", lambda e: e.__setitem__("previous_event_digest", "sha256:" + "0" * 64)),
            ("subject", lambda e: e["subject"].__setitem__("tree", "3" * 40)),
            ("sequence", lambda e: e.__setitem__("sequence", 99)),
            ("worker-authority", lambda e: e["payload"].__setitem__("gate_observation", {"gate_id": "gate-tests"})),
            ("transition", lambda e: e["payload"]["transition"].__setitem__("to", "ACTIVE")),
        ]:
            target = mutated_store(name)
            event_lines = (target / "events.jsonl").read_text().splitlines()
            index = 2 if name == "worker-authority" else (len(event_lines) - 1 if name == "transition" else 3)
            event = json.loads(event_lines[index])
            mutate(event)
            event_lines[index] = json.dumps(event, sort_keys=True, separators=(",", ":"))
        ...
        cases.append(("snapshot drift", lambda: verify_store(snapshot_store, temp_root / "snapshot-drift-receipt.json", "selftest-snapshot-drift")))
        ...
        inspect_receipt, code = recover_store(torn_store, False, temp_root / "torn-inspect.json", "selftest-torn-inspect")
        if code != BAD or inspect_receipt["status"] != "FAIL":
            raise ContractError("torn tail inspection did not fail closed")
        recover_receipt, code = recover_store(torn_store, True, temp_root / "torn-recover.json", "selftest-torn-recover")
        ...
        initialize(contract_path, quota_store, "2026-08-14T00:00:00Z", temp_root / "quota-init.json", "selftest-quota-init")
        for index, event_path in enumerate(event_paths[:5]):
            request_path = temp_root / f"quota-request-{index}.json"
            atomic_write_json(request_path, request_for(load_json(event_path), index, f"quota-request-{index}"))
            append_event(quota_store, request_path, temp_root / f"quota-append-{index}.json", f"selftest-quota-append-{index}")
        quota_event = copy.deepcopy(load_json(event_paths[5]))
        quota_event["payload"]["quota_delta"]["attempts"] = 2
        raw = copy.deepcopy(quota_event); raw.pop("event_digest")
        ...
        append_event(quota_store, quota_request, temp_root / "quota-exhaust-receipt.json", "selftest-quota-exhaust")
        quota_snapshot = load_json(quota_store / "snapshot.json")
        if quota_snapshot["state"]["lifecycle"] != "HITL_PENDING" or quota_snapshot["state"]["todos"][0]["status"] != "HITL_PENDING":
        ...
        raw = copy.deepcopy(terminal_event); raw.pop("event_digest")
        terminal_event["event_digest"] = digest(raw)
        terminal_request = temp_root / "quota-terminal.json"
        atomic_write_json(terminal_request, request_for(terminal_event, 6, "quota-terminal"))
        expect_error(
            lambda: append_event(quota_store, terminal_request, temp_root / "quota-terminal-receipt.json", "selftest-quota-terminal"),
        ...
        try:
            ...
            result = subprocess.run(
                [sys.executable, str(Path(__file__).with_name("ledger.py").resolve()), "append", "--store", str(store), "--request", str(duplicate_request), "--receipt", str(temp_root / "busy-receipt.json"), "--operation-id", "selftest-busy"],
        ...
        expect_error(lambda: validate_contract(load_json(root / "tests" / "fixtures" / "hollow" / "contract.json")), "hollow contract")
        mutations += 1

    print(f"loopx-ledger selftest PASS: 1 positive replay, 1 hollow, {mutations} mutations/controls")
