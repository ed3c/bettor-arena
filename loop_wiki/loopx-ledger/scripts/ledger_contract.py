...
def validate_contract(value: Any) -> dict[str, Any]:
    ...
    for index, gate in enumerate(gates):
        if not isinstance(gate, dict) or gate.get("schema_version") != "loopx/gate-definition/v1":
        ...
        if not isinstance(execution, dict) or "shell" in execution or "raw_command" in execution:
            raise ContractError(f"gate_definitions[{index}] exposes a raw shell")
        gate_ids.add(gate_id)
    command_ids: set[str] = set()
    for index, command in enumerate(commands):
        if not isinstance(command, dict) or command.get("schema_version") != "loopx/command/v1":
            raise ContractError(f"commands[{index}] is not LoopX Contract v1")
        command_id = stable_id(command.get("command_id"), f"commands[{index}].command_id")
        if command_id in command_ids:
            raise ContractError("duplicate command ID")
        if validate_subject(command.get("subject"), f"commands[{index}].subject") != subject:
            raise ContractError("command subject mismatch")
        command_ids.add(command_id)
    state = validate_initial_state(contract["initial_state"], subject, gate_ids)
    if state["state_revision"] != 0 or state["ledger_head_digest"] is not None or state["lifecycle"] != "READY":
        raise ContractError("initial state must be READY at revision zero with no ledger head")
...
def validate_initial_state(value: Any, subject: dict[str, Any], gate_ids: set[str]) -> dict[str, Any]:
    state = exact_object(value, STATE_KEYS, "initial_state")
    if state["schema_version"] != "loopx/task-state/v1" or validate_subject(state["subject"], "initial_state.subject") != subject:
    ...
    sha256_ref(state["ledger_head_digest"], "initial_state.ledger_head_digest", nullable=True)
    if state["lifecycle"] not in {"READY", "ACTIVE", "HITL_PENDING", "COMPLETED", "COMPLETED_WITH_EXCEPTION", "FAILED", "CANCELLED"}:
    ...
    for index, todo in enumerate(state["todos"]):
        ...
        if not isinstance(todo["evidence_refs"], list) or set(todo["evidence_refs"]) - evidence_ids:
            raise ContractError("initial Todo references unknown Evidence")
        if todo["gate_results"] or todo["attempts"] != 0 or todo["last_failure_ref"] is not None or todo["exception_ref"] is not None:
    ...
    quota = state["quota"]
    if not isinstance(quota, dict) or set(quota) != {"limits", "used", "state"} or quota["state"] != "AVAILABLE":
        raise ContractError("initial Quota invalid")
    if not isinstance(quota["used"], dict) or any(type(v) is not int or v != 0 for v in quota["used"].values()):
...
def validate_store_manifest(value: Any, store: Path) -> dict[str, Any]:
    ...
    if manifest["schema_version"] != "loopx/ledger-store/v1" or manifest["writer_policy"] != "POSIX_FLOCK_SINGLE_WRITER":
    ...
    for key in ("contract_path", "events_path", "snapshot_path"):
        ...
        if (store / manifest[key]).resolve().parent != store.resolve() and not str((store / manifest[key]).resolve()).startswith(str(store.resolve()) + os.sep):
...
def validate_append_request(value: Any, subject: dict[str, Any]) -> dict[str, Any]:
    ...
    if type(request["expected_state_revision"]) is not int or request["expected_state_revision"] < 0:
        raise ContractError("append request expected revision invalid")
    event = request["event"]
    if not isinstance(event, dict) or validate_subject(event.get("subject"), "append request.event.subject") != subject:
