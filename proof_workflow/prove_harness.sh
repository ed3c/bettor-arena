...
for p in macro_loop micro_loop openwiki container policy workflow harness notebooklm ctg_loop; do
  case "$p" in
...
# --- every control ----------------------------------------------------------
for c in macro_entry micro_entry openwiki_entry workflow_lineage mcp_surface container_surface sandbox_policy harness_coverage notebooklm_entry ctg_entry; do
