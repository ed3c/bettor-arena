...
# --- CTG carrier: focused reusable public-seam test --------------------------
# Keep the CTG carrier control owned by CTG as well as observed here. Calling a
# focused test avoids making CTG convergence depend on unrelated MCP properties.
capture ctg-inline-carrier -- env CTG_MCP_REF="$PINNED_REF" \
  sh "$ROOT/tests/test_ctg_mcp_carrier.sh"
expect "ctg-inline-carrier-public-seam" $? 0
